...
class KDRandAugment(KDStochasticTransform):
    ...
    def __init__(
        ...
        if isinstance(interpolation, str):
            if interpolation == "random":
                ...
                self.interpolations = [resolve_interpolation("bilinear"), resolve_interpolation("bicubic")]
            else:
                self.interpolations = [resolve_interpolation(interpolation)]
        else:
    ...
    def __call__(self, x, ctx=None):
        assert not paddle.is_tensor(x), "some KDRandAugment transforms require input to be pillow image"
    ...
    def shear_x(self, x, magnitude):
        ...
        # not sure about the equivalent in paddle vision
        # return F.affine(
        #     x,
        #     angle=0.,
        #     translate=[0, 0],
