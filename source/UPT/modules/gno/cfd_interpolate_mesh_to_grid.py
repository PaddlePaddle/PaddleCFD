...
from paddle_geometric.nn.unpool.knn_interpolate import knn_interpolate
from paddle_scatter import segment_csr


class CfdInterpolateMeshToGrid(paddle.nn.Layer):
    @staticmethod
    def forward(x, mesh_pos, grid_pos, batch_idx):
        ...
        batch_y = paddle.arange(batch_size, device=x.device).repeat_interleave(
            num_grid_points
        )
        x = knn_interpolate(
            x=x, pos_x=mesh_pos, pos_y=grid_pos, batch_x=batch_idx, batch_y=batch_y
        )
        return x
