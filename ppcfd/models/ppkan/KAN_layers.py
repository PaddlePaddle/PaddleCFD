...
class KAN_Bspline(nn.Layer):
    def __init__(
        ...
        grid = (
            (paddle.arange(start=-spline_order, end=grid_size + spline_order + 1) * h + grid_range[0])
    ...
    def reset_parameters(self):
        ...
        with paddle.no_grad():
            ...
            if self.enable_standalone_scale_spline:
                init_KaimingUniform = paddle.nn.initializer.KaimingUniform(
                    negative_slope=math.sqrt(5) * self.scale_spline, nonlinearity="leaky_relu"
                )
    ...
    def b_splines(self, x: paddle.Tensor):
        ...
        assert tuple(bases.shape) == (x.shape[0], self.in_features, self.grid_size + self.spline_order)
    ...
    def curve2coeff(self, x: paddle.Tensor, y: paddle.Tensor):
        ...
        assert tuple(result.shape) == (self.out_features, self.in_features, self.grid_size + self.spline_order)
    ...
    def scaled_spline_weight(self):
        return self.spline_weight * (
            self.spline_scaler.unsqueeze(axis=-1) if self.enable_standalone_scale_spline else 1.0
        )

    def forward(self, x: paddle.Tensor):
        assert x.dim() == 2 and x.shape[1] == self.in_features

        base_output = paddle.nn.functional.linear(x=self.base_activation(x), weight=self.base_weight.T)
    ...
    def update_grid(self, x: paddle.Tensor, margin=0.01):
        ...
        grid_adaptive = x_sorted[paddle.linspace(start=0, stop=batch - 1, num=self.grid_size + 1, dtype="int64")]
        uniform_step = (x_sorted[-1] - x_sorted[0] + 2 * margin) / self.grid_size
        grid_uniform = (
            paddle.arange(dtype="float32", end=self.grid_size + 1).unsqueeze(axis=1) * uniform_step
            + x_sorted[0]
            - margin
        )
        ...
        grid = paddle.concat(
            x=[
                grid[:1] - uniform_step * paddle.arange(start=self.spline_order, end=0, step=-1).unsqueeze(axis=1),
                grid,
                grid[-1:] + uniform_step * paddle.arange(start=1, end=self.spline_order + 1).unsqueeze(axis=1),
            ],
            axis=0,
        )

        paddle.assign(grid.T, output=self.grid)
        paddle.assign(self.curve2coeff(x, unreduced_spline_output), output=self.spline_weight.data)
...
class KAN_Legendre(nn.Layer):
    def __init__(self, input_features, output_features, max_degree):
        ...
        self.bias = paddle.base.framework.EagerParamBase.from_tensor(tensor=paddle.zeros(shape=self.output_features))
...
class KAN_Laplace(nn.Layer):
    def __init__(self, input_features, output_features, initial_grid_size, addbias=True):
    ...
    def forward(self, x):
        ...
        y = paddle.sum(x=exp_neg * self.laplace_coeffs[0:1, :, :, :grid_size], axis=(-2, -1))
        y += paddle.sum(x=exp_pos * self.laplace_coeffs[0:1, :, :, :grid_size], axis=(-2, -1))
        if self.addbias:
            y += self.bias
...
class KAN_Fourier(nn.Layer):
    ...
    def forward(self, x):
        ...
        k = paddle.reshape(x=paddle.arange(start=1, end=grid_size + 1), shape=(1, 1, 1, grid_size))
