...
class RadialBasisFunctionLayer(paddle.nn.Layer):
    def __init__(
        ...
        self.grid = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.linspace(start=min_grid, stop=max_grid, num=grid_count, dtype=dtype), trainable=grid_opt
        )
        if not grid_opt:
            self.grid.stop_gradient = True
        self.rbf_weight = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.empty(shape=[in_features * grid_count, out_features], dtype=dtype)
        )

        self.scale_rbf = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.ones(shape=out_features, dtype=dtype)
        )
        ...
        self.scale_base = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.ones(shape=out_features, dtype=dtype)
        )
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        rbf_basis = paddle.exp(
            x=-(((x_unsqueezed - self.grid) / ((self.max_grid - self.min_grid) / (self.grid_count - 1))) ** 2)
        )  # Shape: [batch_size, in_features， grid_count]
        rbf_basis = rbf_basis.contiguous().view(
            rbf_basis.shape[0], -1
        )  # Shape: [batch_size, in_features * grid_count]
        ...
        if self.apply_base_update:
            base_output = paddle.nn.functional.linear(x=self.base_activation(x), weight=self.base_weight.T, bias=None)
...
class RBF_KAN(paddle.nn.Layer):
    def __init__(
        ...
        for in_dim, out_dim in zip(hidden_layers[1:-1], hidden_layers[2:]):
            self.layers.append(
                RadialBasisFunctionLayer(
                    in_dim, out_dim, -1.0, 1.0, grid_count, apply_base_update, activation, grid_opt, noise_scale, dtype
                )
            )
        self.dtype = dtype
...
class KANONet(paddle.nn.Layer):
    def __init__(
        ...
        if trunk_kan_func == "rbf":
            self.trunk_net = RBF_KAN(
                hidden_layers=width_trunk,
                dtype=dtype,
        ...
        if branch_kan_func == "rbf":
            self.branch_net = RBF_KAN(
                hidden_layers=width_branch,
                dtype=dtype,
