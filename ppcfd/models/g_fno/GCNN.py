...
from .GFNO import GConv2d, GConv3d, GMLP2d, GMLP3d, GNorm


class GCNN2d(paddle.nn.Module):
    def __init__(self, num_channels, width, initial_step, reflection):
        ...
        self.pad = partial(
            paddle.compat.nn.functional.pad, pad=[self.padding] * 4, mode="circular"
        )
        self.width = width
        self.p = GConv2d(
...
class GCNN3d(paddle.nn.Module):
    def __init__(self, num_channels, width, initial_step, reflection):
        ...
        self.pad = partial(
            ...
            pad=[0, 0] + [self.padding] * 4,
            mode="circular",
        )
        self.pad0 = partial(paddle.compat.nn.functional.pad, pad=[self.padding] * 2)
        self.width = width
        self.p = GConv3d(
            in_channels=num_channels * initial_step + 2,
            out_channels=self.width,
            kernel_size=1,
            kernel_size_T=1,
            reflection=reflection,
        ...
        self.conv0 = GConv3d(
            ...
            out_channels=self.width,
            kernel_size=self.kernel_size,
            kernel_size_T=self.kernel_size,
            reflection=reflection,
        )
        self.conv1 = GConv3d(
            in_channels=self.width,
