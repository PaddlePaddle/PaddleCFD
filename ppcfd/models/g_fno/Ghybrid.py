...
from .FNO import MLP2d, SpectralConv2d
from .GFNO import GConv2d, GMLP2d, GNorm, GSpectralConv2d


class Ghybrid2d(paddle.nn.Module):
    def __init__(
        self, num_channels, modes, Gwidth, width, initial_step, reflection, n_equiv
    ):
        ...
        self.p = GConv2d(
            ...
            out_channels=self.Gwidth,
            kernel_size=1,
            reflection=reflection,
            first_layer=True,
        )
        assert n_equiv in [1, 2, 3], "Number of equivariant layers should be 1, 2, or 3"
        self.spectral_convs = paddle.nn.ModuleList()
        self.mlps = paddle.nn.ModuleList()
        self.convs = paddle.nn.ModuleList()
        for layer in range(n_equiv):
            self.spectral_convs.append(
                GSpectralConv2d(
                    in_channels=self.Gwidth,
                    out_channels=self.Gwidth,
                    modes=self.modes,
                    reflection=reflection,
                )
        ...
        for layer in range(4 - n_equiv):
            ...
            self.spectral_convs.append(
                SpectralConv2d(in_width, self.width, self.modes, self.modes)
            )
            self.mlps.append(MLP2d(self.width, self.width, self.width))
            self.convs.append(paddle.nn.Conv2d(in_width, self.width, 1))
