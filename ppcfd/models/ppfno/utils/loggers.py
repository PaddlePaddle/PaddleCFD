...
class TensorBoardLogger(Logger):
    ...
    def log_image(self, tag: str, img: Union[TensorType['H', 'W', 'C'], np.
        ndarray], step: int):
...
class WandBLogger(Logger):
    ...
    def __init__(self, project_name: str, run_name: str, log_dir: str,
        config: Optional[Dict]=None, entity: Optional[str]=None):
        super().__init__()
        wandb.init(project=project_name, name=run_name, dir=log_dir, entity
            =entity)
...
def init_logger(config: dict, prefix: str='') ->Logger:
    ...
    for logger_type in config['logger_types']:
        if logger_type == 'tensorboard':
            ...
            loggers.append(WandBLogger(config['project_name'], config[
                'run_name'], entity=config['entity'], log_dir=log_dir,
                config=config))
        else:
...
class TestLoggers(unittest.TestCase):
    ...
    def test_loggers(self):
        loggers = Loggers([TensorBoardLogger('test'), WandBLogger('test',
            'test', 'test')])
