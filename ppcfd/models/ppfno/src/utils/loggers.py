...
class TensorBoardLogger(Logger):
    ...
    def log_image(self, tag: str, img: Union[paddle.Tensor, np.ndarray], step: int):
...
def init_logger(config: dict, prefix: str = "") -> Logger:
    ...
    for logger_type in config["logger_types"]:
        if logger_type == "tensorboard":
            ...
            loggers.append(
                WandBLogger(
                    config["project_name"], config["run_name"], entity=config["entity"], log_dir=log_dir, config=config
                )
            )
        else:
