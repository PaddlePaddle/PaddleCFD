...
class LpLoss(object):
    ...
    def rel(self, x, y):
        ...
        diff_norms = paddle.linalg.norm(
            x=x.reshape((num_examples, -1)) - y.reshape((num_examples, -1)), p=self.p, axis=1
        )
...
class HsLoss(object):
    ...
    def __call__(self, x, y, a=None):
        ...
        k_x = (
            paddle.concat(
                x=(paddle.arange(start=0, end=nx // 2, step=1), paddle.arange(start=-nx // 2, end=0, step=1)), axis=0
            )
            .reshape(nx, 1)
            .tile(repeat_times=[1, ny])
        )
        k_y = (
            paddle.concat(
                x=(paddle.arange(start=0, end=ny // 2, step=1), paddle.arange(start=-ny // 2, end=0, step=1)), axis=0
            )
            .reshape(1, ny)
            .tile(repeat_times=[nx, 1])
        )
        ...
        if balanced is False:
            ...
            if k >= 1:
                ...
                loss += self.rel(x * weight, y * weight)
            if k >= 2:
                weight = a[1] * paddle.sqrt(x=k_x**4 + 2 * k_x**2 * k_y**2 + k_y**4)
                loss += self.rel(x * weight, y * weight)
            loss = loss / (k + 1)
        return loss
