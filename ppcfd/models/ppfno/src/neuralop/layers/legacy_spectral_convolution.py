...
def get_contract_fun(weight, implementation="reconstructed", separable=False):
    ...
    if implementation == "reconstructed":
        ...
        if paddle.is_tensor(x=weight):
            return _contract_dense
        elif isinstance(weight, tltorch.factorized_tensors.core.FactorizedTensor):
    ...
    def __init__(
        ...
        if joint_factorization:
            ...
            self.weight = paddle.nn.LayerList(
                sublayers=[
                    tltorch.factorized_tensors.core.FactorizedTensor.new(
                        weight_shape,
                        rank=self.rank,
                        factorization=factorization,
                        fixed_rank_modes=fixed_rank_modes,
    ...
    def forward(
        ...
        if self.fno_block_precision in ["half", "mixed"]:
            out_fft = paddle.zeros(
                shape=[batchsize, self.out_channels, *fft_size], dtype=paddle.chalf
            )
        else:
            out_fft = paddle.zeros(
