...
def get_contract_fun(weight, implementation="reconstructed", separable=False):
    ...
    if implementation == "reconstructed":
        ...
        if paddle.is_tensor(x=weight):
            return _contract_dense
        elif isinstance(weight, tl.factorized_tensors.core.FactorizedTensor):
...
class SpectralConv(BaseSpectralConv):
    ...
    def __init__(
        ...
        if joint_factorization:
            self.weight = tl.factorized_tensors.core.FactorizedTensor.new(
            ...
            self.weight = paddle.nn.LayerList(
                sublayers=[
                    tl.factorized_tensors.core.FactorizedTensor.new(
                        weight_shape,
                        rank=self.rank,
                        factorization=factorization,
                        fixed_rank_modes=fixed_rank_modes,
        ...
        if bias:
            self.bias = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=init_std * paddle.randn(shape=(n_layers, self.out_channels) + (1,) * self.order)
            )
        else:
            self.bias = None
    ...
    def forward(self, x: paddle.Tensor, indices=0, output_shape: Optional[Tuple[int]] = None):
        ...
        if self.fno_block_precision in ["half", "mixed"]:
            ...
            out_fft = paddle.zeros(shape=[batchsize, self.out_channels, *fft_size], dtype="complex64")
        ...
        for i, boundaries in enumerate(itertools.product(*mode_indexing)):
            ...
            out_fft[idx_tuple] = self._contract(
                x[idx_tuple], self._get_weight(self.n_weights_per_layer * indices + i), separable=self.separable
            )
        if self.output_scaling_factor is not None and output_shape is None:
...
class SpectralConv1d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, width // 2 + 1], dtype="complex64")
        slices = slice(None), slice(None), slice(self.half_n_modes[0])
        out_fft[slices] = self._contract(x[slices], self._get_weight(indices), separable=self.separable)
...
class SpectralConv2d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, height, width // 2 + 1], dtype=x.dtype)
        slices0 = slice(None), slice(None), slice(self.half_n_modes[0]), slice(self.half_n_modes[1])
        """Upper block (truncate high frequencies)."""
        out_fft[slices0] = self._contract(x[slices0], self._get_weight(2 * indices), separable=self.separable)
        slices1 = slice(None), slice(None), slice(-self.half_n_modes[0], None), slice(self.half_n_modes[1])
        """Lower block"""
        out_fft[slices1] = self._contract(x[slices1], self._get_weight(2 * indices + 1), separable=self.separable)
        if self.output_scaling_factor is not None:
...
class SpectralConv3d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, height, width, depth // 2 + 1], dtype="complex64")
        slices0 = (
            slice(None),
            slice(None),
            slice(self.half_n_modes[0]),
            slice(self.half_n_modes[1]),
            slice(self.half_n_modes[2]),
        )
        """Upper block -- truncate high frequencies."""
        out_fft[slices0] = self._contract(x[slices0], self._get_weight(4 * indices + 0), separable=self.separable)
        slices1 = (
            slice(None),
            slice(None),
            slice(self.half_n_modes[0]),
            slice(-self.half_n_modes[1], None),
            slice(self.half_n_modes[2]),
        )
        """Low-pass filter for indices 2 & 4, and high-pass filter for index 3."""
        out_fft[slices1] = self._contract(x[slices1], self._get_weight(4 * indices + 1), separable=self.separable)
        slices2 = (
            slice(None),
            slice(None),
        ...
        out_fft[slices2] = self._contract(x[slices2], self._get_weight(4 * indices + 2), separable=self.separable)
        slices3 = (
            slice(None),
            slice(None),
            slice(-self.half_n_modes[0], None),
        ...
        out_fft[slices3] = self._contract(x[slices3], self._get_weight(4 * indices + 3), separable=self.separable)
        if self.output_scaling_factor is not None:
            width = round(width * self.output_scaling_factor[0])
