...
from ..spherical_convolution import SHT, SphericalConv
...
def test_SphericalConv(factorization, implementation):
    ...
    assert list(tuple(res.shape)[2:]) == [12 * 2, 12 * 2]
    block = SphericalConv(4, 4, n_modes, n_layers=2, sht_grids=["equiangular", "legendre-gauss", "equiangular"])
...
def test_sht(grid):
    ...
    coeffs[:, :lmax, :mmax] = paddle.randn(shape=[batch_size, lmax, mmax], dtype="complex64")
    signal = sht_handle.isht(coeffs, s=(nlat, nlon), grid=grid, norm=norm).to("float32")
