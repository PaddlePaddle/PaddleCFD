...
from utilities3 import memory_usage, num_of_nans, paddle_memory_usage, show_tensor_range
...
class NeighborMLPConvLayerWeighted(paddle.nn.Layer):
    ...
    def forward(
        ...
        rep_csr = fused_segment_csr.select_segment_csr(
            in_features, neighbors_index_int64, neighbors.neighbors_row_splits, reduce=self.reduction
        )
        ...
        if in_weights is None:
            ...
            weights_csr = fused_segment_csr.select_segment_csr(
                in_weights.unsqueeze(axis=-1),
...
class NeighborMLPConvLayerLinear(paddle.nn.Layer):
    ...
    def forward(
        ...
        rep_features = fused_segment_csr.select_segment_csr(
            x_in, neighbors_index_int64, neighbors.neighbors_row_splits, reduce=self.reduction
        )
        ...
        #     neighbors_row_splits, reduce=self.reduction)
        in_features = fused_segment_csr.select_segment_csr(
            in_features.contiguous(), neighbors_index_int64, neighbors.neighbors_row_splits, reduce=self.reduction
        )
