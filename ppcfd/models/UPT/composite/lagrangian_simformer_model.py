...
class LagrangianSimformerModel(CompositeModelBase):
    def __init__(self, encoder, latent, decoder, conditioner=None, **kwargs):
    ...
    def submodels(self):
        return dict(
            **dict(conditioner_encoder=self.conditioner)
            if self.conditioner is not None
            else {},
    ...
    def forward(
        ...
        outputs = {}
        if self.conditioner is not None:
            timestep_embed = self.conditioner(
                timestep=timestep, velocity=paddle.zeros_like(timestep)
            )
        else:
            timestep_embed = None
        ...
        dynamics = self.latent(prev_dynamics, condition=timestep_embed)
        ...
        if reconstruct_prev_target:
            if self.conditioner is not None:
                prev_timestep_embed = self.conditioner(
                    timestep=timestep - 1, velocity=paddle.zeros_like(timestep)
                )
            else:
        ...
        if encode_target:
            if self.conditioner is not None:
                next_timestep_embed = self.conditioner(
                    timestep=timestep + 1, velocity=paddle.zeros_like(timestep)
                )
            else:
                next_timestep_embed = None
            vels = einops.rearrange(
                x,
            ...
            if predict_velocity:
                ...
                current_vel = vels[:, (-1), :]
                current_vel = self.data_container.get_dataset().unnormalize_vel(
                    current_vel
                )
                a = self.data_container.get_dataset().unnormalize_acc(
                    target[perm_batch]
                )
                current_vel = current_vel + a
                current_vel = self.data_container.get_dataset().normalize_vel(
                    current_vel
                )
            new_vels = paddle.concat(
                (vels[:, 1:, :], current_vel.unsqueeze(dim=1)), dim=1
            )
            new_vels = einops.rearrange(
                new_vels,
                "bs_times_n_particles timesteps dim -> bs_times_n_particles (timesteps dim)",
            )
    ...
    def forward_large_t(
        ...
        if self.conditioner is not None:
            if const_timestep:
                timestep = paddle.tensor([0]).to(x.device)
                timestep_embed = self.conditioner(
                    timestep=timestep, velocity=paddle.zeros_like(timestep)
                )
                ...
                next_timestep = (
                    timestep
                    + self.data_container.get_dataset().n_pushforward_timesteps
                    + 1
                )
                next_timestep_embed = self.conditioner(
                    timestep=next_timestep, velocity=paddle.zeros_like(timestep)
                )
        else:
            timestep_embed = None
            next_timestep_embed = None
    ...
    def rollout(
        ...
        for i in range(rollout_length):
            dynamics = self.latent(dynamics, condition=timestep_embed)
            target = self.decoder(
                dynamics,
                query_pos=curr_pos,
                unbatch_idx=unbatch_idx,
            ...
            curr_pos = einops.rearrange(
                curr_pos, "bs n_particles n_dim -> (bs n_particles) n_dim"
            )
            ...
            if self.conditioner is not None:
                ...
                timestep_embed = None
            if full_rollout:
                current_vel_normalized = (
                    self.data_container.get_dataset().normalize_vel(current_vel)
                )
                x = paddle.concat(
                    [vels[:, 1:, :], current_vel_normalized.unsqueeze(1)], dim=1
                )
                x = einops.rearrange(
                    x,
                ...
                mesh_pos = einops.rearrange(
                    curr_pos, "bs n_particles dim -> (bs n_particles) dim"
                )
                dynamics = self.encoder(
                    x,
                    mesh_pos=mesh_pos,
        ...
        all_predictions = paddle.stack(all_predictions, dim=1)
        all_velocities = paddle.stack(all_velocities, dim=1)
    ...
    def rollout_large_t(
        ...
        while True:
            ...
            v_hat = einops.rearrange(
                v_hat, "a (time dim) -> a time dim", dim=curr_pos.shape[-1]
            )
            vel_predictions.append(v_hat)
        vel_predictions = paddle.concat(vel_predictions, dim=1)
