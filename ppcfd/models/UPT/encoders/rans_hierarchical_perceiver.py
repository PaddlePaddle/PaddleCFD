...
class RansHierarchicalPerceiver(SingleModelBase):
    def __init__(
        self, num_stages, dim, depth, num_attn_heads, num_query_tokens, **kwargs
    ):
        ...
        self.models = paddle.nn.LayerList()
        for i in range(num_stages):
            stage_models = paddle.nn.LayerList()
            if i == 0:
                stage_models.append(paddle.nn.Identity())
            else:
                stage_models.append(
                    LinearProjection(self.dim[(i - 1) * 2 + 1], self.dim[i * 2])
                )
            stage_models.append(
                paddle.nn.LayerList(
                    [
                        block_ctor(
                            dim=self.dim[i * 2], num_heads=self.num_attn_heads[i * 2]
                        )
    ...
    def get_model_specific_param_group_modifiers(self):
        return [
            ExcludeFromWdByNameModifier(name=f"models.{i}.3.query")
