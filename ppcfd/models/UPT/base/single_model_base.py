...
class SingleModelBase(ModelBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.optim_ctor = create(
            optim_ctor, optim_ctor_from_kwargs, instantiate_if_ctor=False
        )
        self.freezers = create_collection(
            freezers, freezer_from_kwargs, update_counter=update_counter
        )
        self.is_frozen = is_frozen
        self._is_batch_size_dependent = None
        if self.is_frozen:
            assert (
                self.optim_ctor is None
            ), "model.is_frozen=True but model.optim_ctor is not None"
        assert (
            type(self).before_accumulation_step
            == SingleModelBase.before_accumulation_step
        )
    ...
    def initialize_weights(self):
        if (
            ...
            != ModelBase.model_specific_initialization
        ):
    ...
    def initialize_optim(self, lr_scale_factor=None):
        if self.optim_ctor is not None:
            ...
            if get_trainable_param_count(self) == 0:
                self.logger.info(
                    f"{self.name} has no trainable parameters -> freeze and put into eval mode"
                )
                self.is_frozen = True
                self.eval()
            else:
                raise RuntimeError(
                    f"no optimizer for {self.name} and it's also not frozen"
                )
        else:
