...
class CFDDataTransiton:
    def __init__(self, case_path, save_path, caseID):
        ...
        self.cell_area = None
        self.csv_data = pd.read_csv(os.path.join(case_path, f"{caseID}.csv")).to_numpy()
        ...
        self.const = 2.0 / (self.mass_density * self.flow_speed**2 * self.reference_area)
    ...
    def drag_c(self):
        def pressure_drag_c():
            cell_fp = self.press * self.cell_area * np.sum(self.inward_surface_normal * self.flow_direction, axis=1)
    ...
    def lift_c(self):
        def pressure_lift_c():
            cell_fp = self.press * self.cell_area * np.sum(self.inward_surface_normal * self.lift_direction, axis=1)
    ...
    def generate_visual_vtk(self):
        ...
        meshio.write(os.path.join(self.case_path, f"mesh_{self.caseID[4:].zfill(4)}.vtk"), mesh)
        return None

    def save_values(self):
        np.save(os.path.join(self.save_path, f"pressure_{self.caseID[4:].zfill(4)}.npy"), self.press)
        np.save(os.path.join(self.save_path, f"wallshearstress_{self.caseID[4:].zfill(4)}.npy"), self.wallshearstress)
        np.save(os.path.join(self.save_path, f"centroid_{self.caseID[4:].zfill(4)}.npy"), self.centroid)
        np.save(os.path.join(self.save_path, f"area_{self.caseID[4:].zfill(4)}.npy"), self.area)
        np.save(os.path.join(self.save_path, f"normal_{self.caseID[4:].zfill(4)}.npy"), self.normal)
        info_dict = {
            "length": 0,
            "width": 0,
            "height": 0,
            ...
            "velocity": 65.0,
            "re": 0,
            "reference_area": 0.1866,
            "density": 1.225,
            "compute_normal": False,
        }
        paddle.save(obj=info_dict, path=os.path.join(self.save_path, f"info_{self.caseID[4:].zfill(4)}.pdparams"))
        return None


class MeshTransition:
    def __init__(self, stl_path, save_path, caseID, compute_closest_point=False, geo="mesh"):
    ...
    def combine_stl(self):
        stl_files = [
            f
            for f in os.listdir(self.stl_path + "/surface mesh")
            if os.path.isfile(os.path.join(self.stl_path + "/surface mesh", f))
        ]
        combined_meshes = None
        for stl_file in stl_files:
            current_mesh = mesh.Mesh.from_file(self.stl_path + "/surface mesh/" + stl_file)
    ...
    def refine_stl(self):
        ...
        mesh.apply_filter(
            "meshing_isotropic_explicit_remeshing", iterations=5, targetlen=pymeshlab.PercentageValue(0.03)
        )
        print(
    ...
    def save_mesh(self, combine_stl=False, doRefine=False):
        ...
        if doRefine:
            self.refine_stl()
        stl_mesh = o3d.io.read_triangle_mesh(self.stl_path + f"/{self.caseID}.stl")
        print(stl_mesh)
        stl_mesh.compute_vertex_normals()
        o3d.io.write_triangle_mesh(self.save_path + f"/mesh_{self.caseID[4:].zfill(4)}.ply", stl_mesh)

    def load_bound(self, filename="watertight_global_bounds.txt", eps=1e-06) -> Tuple[List[float], List[float]]:
    ...
    def compute_query_points(self):
        min_bounds, max_bounds = self.load_bound(filename="global_bounds.txt", eps=0.01)
    ...
    def compute_df_closest_point_from_mesh(self):
        stl_mesh = o3d.io.read_triangle_mesh(self.save_path + f"/mesh_{self.caseID[4:].zfill(4)}.ply")
    ...
    def compute_df_closest_point_from_pcd(self):
        ...
        train_point = np.load(os.path.join(self.stl_path, f"centroid_{self.caseID[4:].zfill(4)}.npy"))
    ...
    def save_df_closest_point(self):
        ...
        np.save(os.path.join(self.save_path, f"df_{self.caseID[4:].zfill(4)}.npy"), df_closest_point_dict["df"])
        if df_closest_point_dict["closest"] is not None:
            np.save(
                os.path.join(self.save_path, f"closest_{self.caseID[4:].zfill(4)}.npy"),
...
def compute_save_bounds_all(path, save_path):
    ...
    caseIDs = [d for d in os.listdir(path) if os.path.isdir(os.path.join(path, d))]
    area_bounds_all = []
    global_bounds_all = []
    p_all = []
    wss_all = []
    ...
    global_bounds = [np.min(global_bounds_all[0], axis=0), np.max(global_bounds_all[1], axis=0)]
...
def auto_trans(case_path, stl_path, save_path, caseID):
    ...
    mesh_trans = MeshTransition(stl_path, save_path, caseID, compute_closest_point=False)
