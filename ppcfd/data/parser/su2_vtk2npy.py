...
def su2_to_graph(data, mesh, **kwargs):
    ...
    for key in keys:
        if key == "x":
            paddle.to_tensor(out.Points[:, 0], dtype=dtype)
        elif key == "y":
            paddle.to_tensor(out.Points[:, 1], dtype=dtype)
        elif key == "z":
            paddle.to_tensor(out.Points[:, 2], dtype=dtype)
        else:
            paddle.to_tensor(out.PointData[key], dtype=dtype)
        values.append(value)

    return [nodes, edges, keys, values]


def get_su2mesh_graph(
    mesh_filename: Union[PathLike, str], dtype: np.dtype = np.float32
