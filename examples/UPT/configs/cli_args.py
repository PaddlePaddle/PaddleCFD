...
def _devices(devices):
    ...
    if not devices.isdigit():
        assert all(
            d.isdigit() for d in devices.split(",")
        ), f"specify multiple devices as 0,1,2,3 (not {devices})"
    return devices
...
def parse_run_cli_args() -> CliArgs:
    ...
    parser.add_argument(
        "--accelerator", type=str, default="gpu", choices=["cpu", "gpu"]
    )
    ...
    cudnn_benchmark_group.add_argument(
        "--no_cudnn_benchmark", action="store_false", dest="cudnn_benchmark"
    )
    ...
    cuda_profiling_group.add_argument(
    ...
    sync_batchnorm_group.add_argument(
        "--no_sync_batchnorm", action="store_false", dest="sync_batchnorm"
    )
