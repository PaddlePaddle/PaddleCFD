...
class RansInterpolatedTrainer(SgdTrainer):
    def __init__(self, loss_function, use_sdf=True, max_batch_size=None, **kwargs):
    ...
    def input_shape(self):
        ...
        assert isinstance(
            collator.collator, RansInterpolatedCollator,
        )
        sdf, _ = dataset[0]
    ...
    class Model(paddle.nn.Layer):
        ...
        def prepare(self, batch):
            ...
            return dict(
                x=self.to_device(
                    item="sdf" if self.trainer.use_sdf else "interpolated", batch=batch
                ),
