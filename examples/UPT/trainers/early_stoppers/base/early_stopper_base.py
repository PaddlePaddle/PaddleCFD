...
class EarlyStopperBase:
    def __init__(self, every_n_epochs=None, every_n_updates=None, every_n_samples=None):
        self.logger = logging.getLogger(type(self).__name__)
        assert check_exclusive(
            every_n_epochs, every_n_updates, every_n_samples
    ...
    def should_stop_after_update(self, checkpoint):
        if (
            self.every_n_updates is None
            or checkpoint.update % self.every_n_updates != 0
        ):
            return False
        return self._should_stop()
