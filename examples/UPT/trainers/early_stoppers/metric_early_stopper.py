...
class MetricEarlyStopper(EarlyStopperBase):
    ...
    def _should_stop(self):
        ...
        assert (
            self.metric_key in writer.log_cache
        ), f"couldn't find metric_key {self.metric_key} (valid metric_keys={writer.log_cache.keys()}) -> make sure every_n_epochs/every_n_updates/every_n_samples is aligned with the corresponding callback"
        cur_metric = writer.log_cache[self.metric_key]
        if self._metric_improved(cur_metric):
            self.logger.info(
                f"{self.metric_key} improved: {self.best_metric} --> {cur_metric}"
            )
            ...
            stop_training_str = (
                " --> stop training" if self.tolerance_counter >= self.tolerance else ""
            )
            self.logger.info(
                f"{self.metric_key} stagnated: {self.best_metric} {cmp_str} {cur_metric} ({self.tolerance_counter}/{self.tolerance}){stop_training_str}"
            )
