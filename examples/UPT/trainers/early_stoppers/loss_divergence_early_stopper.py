...
class LossDivergenceEarlyStopper(EarlyStopperBase):
    ...
    def __init__(self, reference_window, tolerance_window, tolerance_factor, **kwargs):
        ...
        assert (
            reference_window is not None and 1 <= reference_window
        ), f"reference_window < 1 ({reference_window})"
        assert (
            tolerance_window is not None and 1 <= tolerance_window
        ), f"tolerance_window < 1 ({tolerance_window})"
        assert (
            tolerance_factor is not None and 1.0 <= tolerance_factor
        ), f"tolerance_factor < 1 ({tolerance_factor})"
    ...
    def _should_stop(self):
        ...
        if tolerance_mean >= reference_mean_with_tolerance:
            ...
            self.logger.info(
                f"reference_window={self.reference_window} reference_mean={reference_mean}"
            )
            self.logger.info(
                f"tolerance_window={self.tolerance_window} tolerance_mean={tolerance_mean}"
            )
            self.logger.info(
                f"tolerance_factor={self.tolerance_factor} reference_mean_with_tolerance={reference_mean_with_tolerance}"
            )
            return True
        return False
