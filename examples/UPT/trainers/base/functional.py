...
def calculate_effective_batch_size_per_device(effective_batch_size, world_size=None):
    ...
    assert (
        effective_batch_size % world_size == 0
...
def calculate_batch_size_and_accumulation_steps(
    effective_batch_size_per_device, max_batch_size=None
):
    if max_batch_size is None:
        batch_size = effective_batch_size_per_device
        accumulation_steps = 1
    elif effective_batch_size_per_device <= max_batch_size:
        batch_size = effective_batch_size_per_device
        accumulation_steps = 1
    else:
...
def calculate_automatic_max_batch_size(
    ...
    for name, submodel in model.submodels.items():
        ...
        for key in sd.keys():
            if key == "state":
                cloned["state"] = {
                    idx_key: {
                        k: (v.clone() if v is not None else v)
                ...
                cloned["param_groups"] = [
                    {k: v for k, v in group.items()} for group in sd["param_groups"]
                ]
            elif key == "param_idx_to_name":
                cloned["param_idx_to_name"] = {
                    k: v for k, v in sd["param_idx_to_name"].items()
                }
            else:
                raise NotImplementedError
        optim_state_dicts[name] = cloned
    batch_sizes = get_powers_of_two(2, effective_batch_size_per_device)
    sample, sample_ctx = next(
        iter(
            paddle.io.DataLoader(
                dataset=train_dataset, batch_size=1, collate_fn=collator
            )
        )
    )
    max_batch_size = 1
    for batch_size in reversed(batch_sizes):
        ...
        if isinstance(sample, (list, tuple)):
            ...
            for item in sample:
                if isinstance(item, (list, tuple)):
                    data.append(
                        [
                            einops.repeat(entry, "1 ... -> bs ...", bs=batch_size)
                            for entry in item
                        ]
                    )
                else:
        ...
        ctx = {
            k: (
                einops.repeat(v, "1 ... -> bs ...", bs=batch_size)
