...
from callbacks.default_callbacks.copy_previous_config_callback import \
    CopyPreviousConfigCallback
from callbacks.default_callbacks.copy_previous_summary_callback import \
    CopyPreviousSummaryCallback
from callbacks.default_callbacks.dataset_stats_callback import \
...
from distributed.config import (get_rank, get_world_size, is_distributed,
                                is_managed, is_rank0)
...
from utils.amp_utils import (get_grad_scaler_and_autocast_context,
...
from utils.seed import get_random_states, set_random_states
from utils.update_counter import UpdateCounter

from .functional import (calculate_automatic_max_batch_size,
                         calculate_batch_size_and_accumulation_steps,
                         calculate_effective_batch_size_per_device)


class SgdTrainer(paddle.nn.Layer):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        # self.device: paddle.device = paddle.device(device)
        self.device = device
        ...
        self.logger.info(
            f"using precision: {self.precision} (desired={precision} backup={backup_precision})"
        )
        self.grad_scaler, self.autocast_context = get_grad_scaler_and_autocast_context(
        ...
        self.train_dataset, self.main_collator = self.data_container.get_dataset(
            "train", mode=self.dataset_mode
        )
        self.main_sampler = self.data_container.get_main_sampler(
            train_dataset=self.train_dataset, **self.main_sampler_kwargs
        )
        eff_len = self.main_sampler.effective_length
        assert (
            eff_len >= self.effective_batch_size
        ), f"{eff_len}<{self.effective_batch_size}"
        ...
        if self.initializer is None:
            ...
            self.start_checkpoint = Checkpoint(
                epoch=start_epoch, update=start_update, sample=start_sample
            )
        else:
    ...
    def input_shape(self):
        ...
        if collator is not None:
            self.logger.warning(
                "infering input_shape with a collator is not supported yet -> collator is ignored"
            )
        multi_view_wrappers = [
            w
            for w in self.train_dataset.all_wrappers
    ...
    def get_all_callbacks(self, model=None):
        if (
            self.end_checkpoint.epoch == 0
            or self.end_checkpoint.update == 0
            or self.end_checkpoint.sample == 0
        ):
    ...
    def get_default_callbacks(self):
        ...
        if not self.update_counter.is_finished:
            ...
            if not is_managed() and is_rank0():
                default_callbacks = [
                    EtaCallback(**default_kwargs, **default_intervals)
                ] + default_callbacks
            default_callbacks += [
                LrCallback(
                    **default_kwargs, every_n_updates=self.track_every_n_updates
                ),
                FreezerCallback(
                    **default_kwargs, every_n_updates=self.track_every_n_updates
                ),
    ...
    def _calculate_batch_size_and_accumulation_steps(self, model, ddp_model):
        self.logger.info(
            f"calculating batch_size and accumulation_steps (effective_batch_size={self.effective_batch_size})"
        )
        assert (
            self.effective_batch_size % get_world_size() == 0
        ), f"effective_batch_size ({self.effective_batch_size}) needs to be multiple of world_size ({get_world_size()})"
        effective_batch_size_per_device = calculate_effective_batch_size_per_device(
            self.effective_batch_size
        )
        if model.is_batch_size_dependent:
            self.logger.info(
                "model is batch_size dependent -> disabled possible gradient accumulation"
            )
        ...
        if is_distributed():
            self.logger.info(
                f"effective_batch_size_per_device: {effective_batch_size_per_device}"
            )
        ...
        if self.max_batch_size is None:
            ...
            max_batch_size = calculate_effective_batch_size_per_device(
                self.max_batch_size
            )
            self.logger.info(
                f"using provided max_batch_size {self.max_batch_size} ({max_batch_size} per device)"
            )
    ...
    def state_dict(self, *args, **kwargs):
        ...
        if is_distributed():
            ...
            random_states_per_device = []
            paddle.distributed.all_gather_object(
                object_list=random_states_per_device,
        ...
        if isinstance(self.grad_scaler, paddle.amp.GradScaler):
    ...
    def load_state_dict(self, state_dict, load_random_states=True):
        ...
        if load_random_states:
            if len(random_states) != get_world_size():
                self.logger.warning(
                    f"trainer checkpoint has different world_size (ckpt_world_size={len(random_states)} world_size={get_world_size()}) -> can't load random states"
                )
            else:
    ...
    def apply_resume_initializer(self, model):
        if self.initializer is not None:
            ...
            self.logger.info(
                f"loading state from checkpoint {self.initializer.stage_id}/{self.initializer.stage_name}/{self.initializer.checkpoint}"
            )
    ...
    def get_data_loader(self, periodic_callbacks, batch_size):
        ...
        for c in periodic_callbacks:
            ...
            for cur_config in cur_configs:
                ...
                self.logger.info(
                    f"{c} registered {cur_config} dataset_mode='{dataset_mode}'"
                )
            configs += cur_configs
        kwargs = {}
    ...
    def wrap_model(self, model):
        ...
        ), "Model needs to be initialized before DDP wrapping as DPP broadcasts params"
    ...
    def wrap_ddp(self, model):
        if is_distributed():
            if get_trainable_param_count(model) > 0:
                ...
                if "cpu" not in paddle.get_device() and self.sync_batchnorm:
                    model = paddle.nn.SyncBatchNorm.convert_sync_batchnorm(layer=model)
            else:
                self.logger.info(
                    f"not wrapping into DDP (no trainable parameters) -> only broadcast parameters"
                )
                first_param = next(model.parameters())
                first_param.stop_gradient = not True
                paddle.DataParallel(layers=model)
                first_param.stop_gradient = not False
        return model
    ...
    def train_model(self, model, callbacks=None):
        ...
        periodic_callbacks = [
            callback for callback in callbacks if isinstance(callback, PeriodicCallback)
        ]
        ...
        ) = self._prepare_batch_size(model, ddp_model)
        assert trainer_model.model == model
        trainer_model = trainer_model.to(model.device)
        data_loader = self.get_data_loader(
            periodic_callbacks=periodic_callbacks, batch_size=batch_size
        )
        self.call_before_training(
            trainer_model=trainer_model, batch_size=batch_size, callbacks=callbacks
        )
        self._train(
            model=model,
    ...
    def _train(
        ...
        if self.update_counter.is_finished:
            if not model.is_frozen:
                self.logger.warning(
                    "model has optimizer which is not used for evaluation"
                )
            ...
            while True:
                ...
                while True:
                    ...
                    if remaining_batches < accumulation_steps:
                        for _ in range(remaining_batches):
                            _ = next(data_iter)
                        break
                    is_last_update_in_epoch = (
                        remaining_batches - accumulation_steps < accumulation_steps
                    )
                    for callback in periodic_callbacks:
                        callback.before_every_update(
                            update_counter=self.update_counter, model=model
                        )
                    ...
                    if self.update_counter.is_finished:
                        ...
                        if data_loader.batch_sampler.sampler.samples is not None:
                            total_batches = int(
                                data_loader.batch_sampler.sampler.samples / batch_size
                            )
                    ...
                    if self.early_stopper is not None:
                        should_stop_after_update = (
                            self.early_stopper.should_stop_after_update(
                                self.update_counter.cur_checkpoint
                            )
                        )
                        if should_stop_after_update:
                            return
                        should_stop_after_sample = (
                            self.early_stopper.should_stop_after_sample(
                                self.update_counter.cur_checkpoint,
                                effective_batch_size=self.effective_batch_size,
                            )
                        )
                    ...
                    if self.stop_at_update is not None:
                        if self.stop_at_update <= self.update_counter.update:
                            self.logger.info(
                                f"reached stop_at_update (={self.stop_at_update}) -> stop training"
                            )
                            return
                    if self.stop_at_sample is not None:
                        if self.stop_at_sample <= self.update_counter.sample:
                            self.logger.info(
                                f"reached stop_at_sample (={self.stop_at_sample}) -> stop training"
                            )
                            return
                if self.update_counter.is_full_epoch:
                    ...
                    if self.early_stopper is not None:
                        if self.early_stopper.should_stop_after_epoch(
                            self.update_counter.cur_checkpoint
                        ):
                            return
                    if self.stop_at_epoch is not None:
                        if self.stop_at_epoch <= self.update_counter.epoch:
                            self.logger.info(
                                f"reached stop_at_epoch (={self.stop_at_epoch}) -> stop training"
                            )
                            return
                    CallbackBase.flush()
                if self.update_counter.is_finished:
        ...
        try:
            next(data_iter)
            self.logger.error(
                "data_iter was not fully consumed -> checking how many batches remain"
            )
            ...
            for _ in range(10):
                ...
                unconsumed_data_iter_steps += 1
            raise RuntimeError(
                f"data_iter was not fully consumed (at least {unconsumed_data_iter_steps} unconsumed)"
            )
        except StopIteration:
            if unconsumed_data_iter_steps > 0:
                raise RuntimeError(
                    f"data_iter was not fully consumed ({unconsumed_data_iter_steps} unconsumed)"
                )
    ...
    def update(
        ...
        with kp.named_profile_async("forward"):
            if self.precision == paddle.float16:
                autocast_ctx = paddle.amp.auto_cast(enable=True, dtype="float16")
            elif self.precision == paddle.bfloat16:
                autocast_ctx = paddle.amp.auto_cast(enable=True, dtype="bfloat16")
            else:
                autocast_ctx = self.autocast_context
            with autocast_ctx:
                losses, outputs = ddp_model(batch)
            total_loss = losses["total"] / accumulation_steps
        if not model.is_frozen:
            if self.exit_on_nan_loss and paddle.isnan(total_loss):
    ...
    def _prepare_batch_size(self, model, ddp_model):
        ...
        (
            batch_size,
            accumulation_steps,
        ) = self._calculate_batch_size_and_accumulation_steps(model, ddp_model)
        if accumulation_steps > 1 and self.end_checkpoint.update is not None:
            raise NotImplementedError(
                "InterleavedSampler counts every batch as update -> accumulation steps not supported with update-based end_checkpoint"
            )
        ...
        self.logger.info(
            f"train_batches per epoch: {train_batches_per_epoch} (world_size={get_world_size()} batch_size={batch_size})"
        )
