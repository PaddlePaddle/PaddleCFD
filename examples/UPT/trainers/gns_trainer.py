    ...
    class Model(paddle.nn.Layer):
        ...
        def forward(self, batch, reduction="mean"):
            ...
            target_acc = einops.rearrange(
                target_acc, "bs n_particles n_dim -> (bs n_particles) n_dim"
            )
            prev_acc = einops.rearrange(
                prev_acc, "bs n_particles n_dim -> (bs n_particles) n_dim"
            )
            ...
            losses = dict(
                a_hat=self.trainer.loss_function(
                    prediction=model_outputs, target=target_acc, reduction=reduction
                )
            )
