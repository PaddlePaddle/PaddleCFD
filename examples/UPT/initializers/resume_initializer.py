...
class ResumeInitializer(CheckpointInitializer):
    ...
    def _init_weights(self, name, model):
        if isinstance(model, SingleModelBase):
            model_name, ckpt_uri = self._get_modelname_and_ckpturi(
                model=model, model_name=name, file_type="model"
            )
            sd = paddle.load(path=str(ckpt_uri))
            if "state_dict" in sd:
                sd = sd["state_dict"]
    ...
    def _init_optim(self, name, model):
        if isinstance(model, SingleModelBase):
            if model.optim is None:
                self.logger.info(
                    f"skip loading optim from checkpoint '{self.checkpoint}' for {model.name} (optim is None)"
                )
            elif model.is_frozen:
                self.logger.info(
                    f"skip loading optim from checkpoint '{self.checkpoint}' for {model.name} (is_frozen)"
                )
            else:
                model_name, ckpt_uri = self._get_modelname_and_ckpturi(
    ...
    def get_start_checkpoint(self):
        if isinstance(self.checkpoint, str):
            ...
            if trainer_ckpt_uri.exists():
                trainer_ckpt = paddle.load(path=str(trainer_ckpt_uri))
                trainer_ckpt_without_rng_states = {
                    k: v for k, v in trainer_ckpt.items() if k != "random_states"
                }
                self.logger.info(
                    f"loaded checkpoint from trainer_state_dict: {trainer_ckpt_without_rng_states}"
                )
                return Checkpoint(
                ...
                self.logger.warning(
                    "no trainer checkpoint found -> try to fetch start_checkpoint from a model ckpt"
                )
                ckpt_folder = self.path_provider.get_stage_checkpoint_path(
                    stage_name=self.stage_name, stage_id=self.stage_id
                )
                fnames = [
                    fname
                ...
                assert (
                    len(fnames) > 0
                ), "no trainer checkpoint and no valid model found to infer start_checkpoint"
                fname = fnames[0]
                self.logger.info(
                    f"no trainer checkpoint found but start_checkpoint can be inferred from model checkpoint '{fname}'"
                )
                model_sd = paddle.load(path=str(ckpt_folder / fname))
    ...
    def init_trainer(self, trainer):
        ...
        if not ckpt_uri.exists():
            self.logger.warning(
                f"no trainer checkpoint found -> skip trainer initialization from checkpoint"
            )
            return
        trainer.load_state_dict(
            paddle.load(path=str(ckpt_uri)), load_random_states=self.load_random_states
        )
