...
class PretrainedInitializer(InitializerBase):
    ...
    def __init__(self, weights_file, root_key=None, key_mapping=None, **kwargs):
        ...
        self.weights_uri = Path(
            self.path_provider.model_path / weights_file
        ).expanduser()
        assert (
            self.weights_uri.exists() and self.weights_uri.is_file()
        ), self.weights_uri.as_posix()
    ...
    def _get_model_kwargs(self):
        ...
        sd = paddle.load(path=str(self.weights_uri))
    ...
    def init_weights(self, model):
        ...
        if isinstance(model, MaskedDecoder) and self.weights_file in [
            ...
            "mae_large16.pth",
            "mae_huge14.pth",
            "mae_base16res448.pth",
            "mae_large16res448.pth",
            "mae_base16res448e800.pth",
            "mae_large16res448e800.pth",
        ]:
            for key in sd.keys():
                print(key)
            ...
            sd = {
                k: v for k, v in sd.items() if "decoder" not in k and k != "mask_token"
            }
