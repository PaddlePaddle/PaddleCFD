...
class CheckpointInitializer(InitializerBase):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(checkpoint, str):
            ...
            assert (
                self.checkpoint.is_minimally_specified
                or self.checkpoint.is_fully_specified
            )
    ...
    def _get_modelname_and_ckpturi(self, file_type, model=None, model_name=None):
        ...
        ckpt_uri = self._get_ckpt_uri(
            prefix=f"{model_name} cp=", suffix=f"{model_info_str} {file_type}.th"
        )
    ...
    def _get_ckpt_uri(self, prefix, suffix):
        ...
        if (
            ...
            and not self.checkpoint.is_fully_specified
        ):
