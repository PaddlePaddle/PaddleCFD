...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="best_model.loss.online.x_hat.E1"
    )
    parser.add_argument(
        "--mode",
        type=str,
        default="correlation",
        choices=["gif", "loss", "correlation"],
    )
...
def main(
    ...
    if "grid_resolution" in run.config["datasets"]["train"]:
        if "standardize_query_pos" in run.config["datasets"]["train"]:
            print(
                f"found grid_resolution and standardize_query_pos -> using interpolated template"
            )
    ...
    if version is None:
        ...
        name += f"-{name}"
    hp["vars"]["num_input_timesteps"] = run.config["datasets"]["train"][
        "num_input_timesteps"
    ]
    if (
        num_input_points_ratio is None
        and "num_input_points_ratio" not in run.config["datasets"]["train"]
    ):
        hp["vars"]["num_input_points_ratio"] = None
    elif num_input_points_ratio is not None:
        hp["vars"]["num_input_points_ratio"] = num_input_points_ratio
    else:
        hp["vars"]["num_input_points_ratio"] = run.config["datasets"]["train"][
            "num_input_points_ratio"
        ]
    if (
        num_input_points is None
        and "num_input_points" not in run.config["datasets"]["train"]
    ):
        hp["vars"]["num_input_points"] = None
    elif num_input_points is not None:
        hp["vars"]["num_input_points"] = num_input_points
    elif isinstance(run.config["datasets"]["train"]["num_input_points"], dict):
        assert "test_rollout" in run.config["datasets"]
        hp["vars"]["num_input_points"] = run.config["datasets"]["test_rollout"][
            "num_input_points"
        ]
    else:
        hp["vars"]["num_input_points"] = run.config["datasets"]["train"][
            "num_input_points"
        ]
    if "radius_graph_r" in run.config["datasets"]["train"]:
        ...
        radius_graph_max_num_neighbors = (
            k or run.config["datasets"]["train"]["radius_graph_max_num_neighbors"]
        )
        ...
        hp["vars"]["radius_graph_max_num_neighbors"] = (
            k or run.config["trainer"]["radius_graph_max_num_neighbors"]
        )
    ...
    if "max_num_timesteps" in run.config["datasets"]["train"]:
        hp["vars"]["max_num_timesteps"] = run.config["datasets"]["train"][
            "max_num_timesteps"
        ]
    else:
    ...
    if num_supernodes is None:
        if "num_supernodes" in run.config["datasets"]["train"]:
            hp["vars"]["num_supernodes"] = run.config["datasets"]["train"][
                "num_supernodes"
            ]
        elif "num_supernodes" in run.config["datasets"]["train"]["collators"]["0"]:
            hp["vars"]["num_supernodes"] = run.config["datasets"]["train"]["collators"][
                "0"
            ]["num_supernodes"]
        elif "num_supernodes" in hp["vars"]:
