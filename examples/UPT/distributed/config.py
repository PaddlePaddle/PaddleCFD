...
def is_mpi_managed_run():
    return (
        ...
        and "OMPI_MCA_orte_num_nodes" in os.environ
    )


def is_custom_managed_run():
    return (
        "CUSTOM_NUM_NODES" in os.environ
        and "CUSTOM_WORLD_SIZE" in os.environ
        and "CUSTOM_RANK" in os.environ
        and "CUSTOM_LOCAL_RANK" in os.environ
    )
...
def get_backend():
    if is_distributed():
        return paddle.distributed.get_backend()
    return None
