...
def patchify_as_1d(x, patch_size):
    ...
    x = einops.rearrange(
        x, f"bs {from_pattern} -> bs ({to_pattern1}) ({to_pattern2})", **kwargs
    )
    return x
...
def unpatchify(patches, patch_size, img_shape=None):
    if patches.ndim == 3:
        return unpatchify_from_1d(
            patches=patches, patch_size=patch_size, img_shape=img_shape
        )
    elif patches.ndim == 4:
...
def unpatchify_from_1d(patches, patch_size, img_shape=None):
    ...
    if img_shape is not None:
        ...
        seqlen_h = seqlen_w = int(patches.shape[1] ** 0.5)
    img = einops.rearrange(
        patches,
