...
def png_loader_full(path):
    with open(path, "rb") as f:
        return paddle.vision.transforms.to_tensor(Image.open(f))


def png_loader_viridis(path):
    img = (png_loader_full(path) * 255).squeeze(0).long().numpy()
    rgb = np.take(VIRIDIS_NP, img, axis=0)
    return einops.rearrange(paddle.from_numpy(rgb), "h w c -> c h w")
...
def png_loader_full_with_info(path):
    ...
    for row in reader:
        ...
        data.append(paddle.to_tensor(np.frombuffer(np.array(buffer), dtype=dtype)))
    data = paddle.stack(data)
