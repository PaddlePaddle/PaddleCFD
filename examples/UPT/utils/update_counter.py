...
class UpdateCounter:
    def __init__(
        ...
        assert self.start_checkpoint == Checkpoint(
            epoch=self.start_checkpoint.epoch
        ).to_fully_specified(
            updates_per_epoch=updates_per_epoch,
            effective_batch_size=effective_batch_size,
        )
        assert end_checkpoint.is_minimally_specified
        delta_ckpt = end_checkpoint - self.start_checkpoint.to_target_specification(
            end_checkpoint
        )
    ...
    def is_finished(self):
        return (
            self.cur_checkpoint.to_target_specification(self.end_checkpoint)
            >= self.end_checkpoint
        )
