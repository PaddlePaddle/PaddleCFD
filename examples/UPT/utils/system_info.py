...
from distributed.config import get_local_rank, get_rank
...
def get_installed_cuda_version():
    ...
    for line in nvidia_smi_lines:
        if "CUDA Version:" in line:
            return line[
                line.index("CUDA Version: ") + len("CUDA Version: ") : -1
            ].strip()
    return None
...
def log_system_info():
    ...
    if git_hash_file.exists():
        ...
        logging.warning(
            "could not retrieve current git commit hash from ./.git/FETCH_HEAD"
        )
    with log_from_all_ranks():
        logging.info(
            f"initialized process rank={get_rank()} local_rank={get_local_rank()} pid={os.getpid()} hostname={platform.uname().node}"
        )
