...
class Checkpoint:
    ...
    def specified_properties_count(self):
        return sum(
            [self.epoch is not None, self.update is not None, self.sample is not None]
        )
    ...
    def has_same_specified_properties(self, other):
        if not (self.epoch is None) == (other.epoch is None):
            return False
        if not (self.update is None) == (other.update is None):
            return False
        if not (self.sample is None) == (other.sample is None):
            return False
        return True
    ...
    def from_checkpoint_string(checkpoint_string):
        ...
        return Checkpoint(
            epoch=int(epoch_str), update=int(update_str), sample=int(sample_str)
        )
    ...
    def to_fully_specified_from_fnames(ckpt_folder, ckpt, prefix=None, suffix=None):
        ...
        for f in os.listdir(ckpt_folder):
            ...
            if not Checkpoint.contains_checkpoint_string(f):
                continue
            ckpt_from_fname = Checkpoint.from_checkpoint_string(
                Checkpoint.find_checkpoint_string(f)
            )
            if ckpt_from_fname.to_target_specification(ckpt) == ckpt:
                return ckpt_from_fname
        raise FileNotFoundError(
            f"no checkpoint file found (folder='{ckpt_folder}' checkpoint='{ckpt}' prefix='{prefix}' suffix='{suffix}')"
        )
