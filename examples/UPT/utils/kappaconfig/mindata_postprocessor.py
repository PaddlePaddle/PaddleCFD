...
from .testrun_constants import (TEST_RUN_EFFECTIVE_BATCH_SIZE,
                                TEST_RUN_UPDATES_PER_EPOCH)
        ...
        if isinstance(parent_accessor, str):
            if parent_accessor == "main_sampler_kwargs":
                if "weighted_size" in node:
                    node["weighted_size"] = (
                        TEST_RUN_EFFECTIVE_BATCH_SIZE * TEST_RUN_UPDATES_PER_EPOCH
                    )
            if parent_accessor == "datasets":
                for key in node.keys():
                    ...
                    wrappers = [
                        dict(kind="shuffle_wrapper", seed=0),
                        dict(
                            kind="subset_wrapper",
                            end_index=TEST_RUN_EFFECTIVE_BATCH_SIZE
                            * TEST_RUN_UPDATES_PER_EPOCH
                            + 1,
                        ),
                    ]
                    if "dataset_wrappers" in node[key]:
                        ...
                        assert isinstance(
                            node[key], dict
                        ), "found non-dict value inside 'datasets' node -> probably wrong template parameter (e.g. template.version instead of template.vars.version)"
                        node[key]["dataset_wrappers"] = wrappers
            elif parent_accessor in [
                "effective_batch_size",
                "effective_labeled_batch_size",
            ]:
                parent[parent_accessor] = min(
                    parent[parent_accessor], TEST_RUN_EFFECTIVE_BATCH_SIZE
                )
            elif parent_accessor == "optim":
                parent[parent_accessor]["lr_scaler"] = dict(
                    kind="linear_lr_scaler", divisor=TEST_RUN_EFFECTIVE_BATCH_SIZE
                )
