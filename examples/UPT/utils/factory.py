    ...
    if instantiate_if_ctor and isinstance(obj_or_kwargs, (partial, type)):
        if isinstance(obj_or_kwargs, partial):
            for key in kwargs.keys():
                assert (
                    key not in obj_or_kwargs.keywords
...
def create_collection(collection, from_kwargs_fn, collate_fn=None, **kwargs):
    if isinstance(collection, list):
        ...
        objs = {
            key: create(ckwargs, from_kwargs_fn, **kwargs)
        ...
        raise NotImplementedError(
            f"invalid collection type {type(collection).__name__} (expected dict or list)"
        )
...
def type_from_name(module_names, type_names, error_on_not_found=True):
    ...
    if error_on_not_found:
        raise RuntimeError(
            f"can't find class {' or '.join(type_names)} in {' or '.join(module_names)}"
        )
    else:
        return None
...
def _get_type_from_module(module, type_name):
    if type_name in module.__dict__:
        return getattr(module, type_name)
    type_name_lowercase = type_name.lower().replace("_", "")
    possible_type_names = list(
        filter(lambda k: k.lower() == type_name_lowercase, module.__dict__.keys())
    )
    if len(possible_type_names) > 1:
        possible_type_names = [
            name for name in possible_type_names if not name.isupper()
        ]
    assert (
        len(possible_type_names) <= 1
    ), f"error found more than one possible type for {type_name_lowercase}"
    if len(possible_type_names) == 1:
