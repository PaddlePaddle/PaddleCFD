...
class ShapenetCar(DatasetBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.domain_min = paddle.to_tensor([-2.0, -1.0, -4.5], dtype='float32')
        self.domain_max = paddle.to_tensor([2.0, 4.5, 6.0], dtype='float32')
        self.scale = 200
        self.standardize_query_pos = standardize_query_pos
        self.concat_pos_to_sdf = concat_pos_to_sdf
        self.mean = paddle.to_tensor(-36.3099)
        self.std = paddle.to_tensor(48.5743)
        global_root, local_root = self._get_roots(
            global_root, local_root, "shapenet_car"
        )
        if local_root is None:
            ...
            if is_data_rank0():
                ...
                if not self.source_root.exists():
                    self.logger.info(
                        f"copying {(global_root / 'preprocessed').as_posix()} to {(self.source_root / 'preprocessed').as_posix()}"
                    )
                    shutil.copytree(
                        global_root / "preprocessed", self.source_root / "preprocessed"
                    )
        ...
        assert (
            self.source_root.name == "preprocessed"
        ), f"'{self.source_root.as_posix()}' is not preprocessed folder"
        self.uris = []
        for i in range(9):
    ...
    def getitem_pressure(self, idx, ctx=None):
        p = paddle.load(path=str(self.uris[idx] / "pressure.th"))
        # p = p.cpu()
        p -= self.mean
        p /= self.std
        return p
    ...
    def getitem_grid_pos(self, idx=None, ctx=None):
        ...
        x_linspace = paddle.linspace(0, self.scale, self.grid_resolution[0])
        y_linspace = paddle.linspace(0, self.scale, self.grid_resolution[1])
        z_linspace = paddle.linspace(0, self.scale, self.grid_resolution[2])
        meshgrid = paddle.meshgrid(x_linspace, y_linspace, z_linspace, indexing="ij")
        grid_pos = paddle.stack(meshgrid).flatten(start_dim=1).T
        if ctx is not None:
    ...
    def getitem_grid_to_query_edges(self, idx, ctx=None):
        ...
        edges = radius(
            ...
            y=query_pos,
            r=self.radius_graph_r,
            max_num_neighbors=int(10000000000.0),
        ).T
        return edges
    ...
    def getitem_mesh_pos(self, idx, ctx=None):
        ...
        if self.num_input_points_ratio is not None:
            ...
            if self.seed is not None:
                generator = paddle.Generator().manual_seed(self.seed + int(idx))
            else:
                generator = None
            if self.num_input_points_ratio[0] == self.num_input_points_ratio[1]:
                ...
                num_input_points_ratio = paddle.rand(size=(1,)).item() * (ub - lb) + lb
                end = int(len(mesh_pos) * num_input_points_ratio)
            perm = paddle.randperm(len(mesh_pos))[:end]
    ...
    def getitem_all_pos(self, idx, ctx=None):
        if ctx is not None and "all_pos" in ctx:
            return ctx["all_pos"]
        all_pos = paddle.load(path=str(self.uris[idx] / "mesh_points.th"))
        # all_pos.sub_(self.domain_min).div_(self.domain_max - self.domain_min).mul_(
        #     self.scale
        # )
        all_pos = (all_pos - self.domain_min) / (self.domain_max - self.domain_min) * self.scale
        assert paddle.all(0 < all_pos)
        assert paddle.all(all_pos < self.scale)
        if ctx is not None:
            ctx["all_pos"] = all_pos
        return all_pos
    ...
    def getitem_query_pos(self, idx, ctx=None):
        ...
        if self.num_query_points_ratio is not None:
            ...
            perm = paddle.randperm(len(query_pos))[:end]
    ...
    def getitem_sdf(self, idx, ctx=None):
        assert self.grid_resolution is not None
        assert all(
            self.grid_resolution[0] == grid_resolution
            for grid_resolution in self.grid_resolution[1:]
        )
        sdf = paddle.load(
            path=str(self.uris[idx] / f"sdf_res{self.grid_resolution[0]}.th")
        )
        if self.concat_pos_to_sdf:
            x_linspace = paddle.linspace(-1, 1, self.grid_resolution[0])
            y_linspace = paddle.linspace(-1, 1, self.grid_resolution[1])
            z_linspace = paddle.linspace(-1, 1, self.grid_resolution[2])
            grid_pos = paddle.meshgrid(
                x_linspace, y_linspace, z_linspace, indexing="ij"
            )
            sdf = paddle.stack([sdf, *grid_pos], axis=-1)
        else:
            sdf = sdf.unsqueeze(-1)
        return sdf
    ...
    def getitem_interpolated(self, idx, ctx=None):
        ...
        y_linspace = paddle.linspace(0, self.scale, self.grid_resolution[1])
        z_linspace = paddle.linspace(0, self.scale, self.grid_resolution[2])
        grid_pos = paddle.meshgrid(x_linspace, y_linspace, z_linspace, indexing="xy")
        grid = paddle.from_numpy(
            scipy.interpolate.griddata(
                mesh_pos.unbind(1),
                paddle.ones_like(mesh_pos),
                grid_pos,
                method="linear",
                fill_value=0.0,
            )
