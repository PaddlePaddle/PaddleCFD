...
class LagrangianDataset(DatasetBase):
    def __init__(
        ...
        knn_graph_k=1,
        radius_graph_r=0.05,
        radius_graph_max_num_neighbors=int(10000000000.0),
        split="train",
        ...
        **kwargs,
        ...
        assert (
            n_input_timesteps > 1
        ), f"num_inputs_timesteps must be greater than 1 to calculate input velocities."
        assert graph_mode in [
            "knn",
            "radius_graph",
            "radius_graph_with_supernodes",
        ], f"graph_mode {graph_mode} not available."
        ...
        global_root, local_root = self._get_roots(
            global_root, local_root, "lagrangian_dataset"
        )
        ...
        self.vel_mean = paddle.tensor(self.metadata["vel_mean"])
        self.vel_std = paddle.tensor(self.metadata["vel_std"])
        self.acc_mean = paddle.tensor(self.metadata["acc_mean"])
        self.acc_std = paddle.tensor(self.metadata["acc_std"])
        if any(self.metadata["periodic_boundary_conditions"]):
            bounds = paddle.tensor(self.metadata["bounds"])
        ...
        if self.split == "train":
            ...
            self.n_per_traj = (
                self.n_seq - self.n_input_timesteps - self.n_pushforward_timesteps
            )
    ...
    def get_window(self, idx: int, ctx=None, downsample=False):
        ...
        if self.num_points_range:
            if self.num_points_range[0] == self.num_points_range[1]:
                ...
                num_points_range = paddle.rand(size=(1,)).item() * (ub - lb) + lb
                end = int(num_points_range)
            perm = paddle.randperm(self.n_particles)[:end]
            ctx["perm"] = perm
    ...
    def get_trajectory(self, idx: int, ctx=None, downsample=False):
        ...
        positions = traj["position"][ctx["time_idx"]]
        particle_types = traj["particle_type"]
        return paddle.tensor(positions), paddle.tensor(particle_types)

    def get_full_trajectory(self, idx: int, ctx=None, downsample=False):
        i_traj = idx
        traj = self.data[self.traj_keys[i_traj]]
        ctx["time_idx"] = paddle.arange(0, len(traj["position"]))
        ctx["traj_idx"] = i_traj
        positions = traj["position"][ctx["time_idx"]]
        particle_types = traj["particle_type"]
        return paddle.tensor(positions), paddle.tensor(particle_types)
    ...
    def get_accelerations(self, positions):
        ...
        if self.box is not None:
            next_velocities = (
                next_velocities + self.box * 0.5
            ) % self.box - 0.5 * self.box
            current_velocities = (
                current_velocities + self.box * 0.5
            ) % self.box - 0.5 * self.box
    ...
    def getshape_timestep(self):
        return (
            max(
                ...
                self.metadata["sequence_length_test"],
            ),
        )
    ...
    def getitem_edge_index(self, idx, ctx=None, downsample=True):
        ...
        if self.graph_mode == "knn":
            ...
            perm_supernodes = paddle.randperm(current_input_position.shape[0])[
                : self.n_supernodes
            ]
    ...
    def getitem_edge_index_target(self, idx, ctx=None, downsample=True):
        ...
        if self.graph_mode == "knn":
            ...
            generator = self._get_generator(idx)
            perm_supernodes = paddle.randperm(target_position.shape[0])[
                : self.n_supernodes
            ]
    ...
    def getitem_edge_features(self, idx, ctx=None, downsample=True):
        ...
        relative_displacement = (
            target_position[edge_index[:, (0)]] - target_position[edge_index[:, (1)]]
        )
        distance = (
            target_position[edge_index[:, (0)]] - target_position[edge_index[:, (1)]]
        ).norm(dim=-1)
        return paddle.concat([relative_displacement, distance.unsqueeze(dim=-1)], dim=1)
    ...
    def getitem_perm(self, idx, ctx=None):
        if ctx and "perm" in ctx:
            ...
            return paddle.arange(self.n_particles), self.n_particles
