...
class DatasetConfigProvider:
    def __init__(
        self, global_dataset_paths, local_dataset_path=None, data_source_modes=None
    ):
    ...
    def get_data_source_mode(self, dataset_identifier):
        if self.get_local_dataset_path() is None:
            return "global"
        if (
            self.data_source_modes is None
            or dataset_identifier not in self.data_source_modes
        ):
            return None
        data_source_mode = self.data_source_modes[dataset_identifier]
        assert data_source_mode in [
            "global",
            "local",
