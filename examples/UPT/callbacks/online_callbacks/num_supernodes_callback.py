...
class NumSupernodesCallback(PeriodicCallback):
    ...
    def _track_after_accumulation_step(self, **kwargs):
        ...
        if self.is_first_log:
            self.logger.info(
                f"num_nodes: per_device={self.num_nodes_history[-1]} per_sample={self.num_nodes_history[-1] // self.trainer_batch_size}"
            )
            self.logger.info(
                f"num_supernodes: per_device={self.num_supernodes_history[-1]} per_sample={self.num_supernodes_history[-1] // self.trainer_batch_size}"
            )
