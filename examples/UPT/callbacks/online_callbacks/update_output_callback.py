...
from distributed.gather import all_gather_nograd, all_reduce_mean_grad
...
class UpdateOutputCallback(PeriodicCallback):
    ...
    def _periodic_callback(self, **_):
        for key, tracked_values in self.tracked_values.items():
            if self.reduce == "mean":
                if paddle.is_tensor(tracked_values[0]):
                    reduced_value = paddle.stack(tracked_values).float().mean()
                else:
                    reduced_value = float(np.mean(tracked_values))
                reduced_value = all_reduce_mean_grad(reduced_value)
            elif self.reduce == "last":
                reduced_value = all_gather_nograd(paddle.concat(tracked_values))
            else:
                raise NotImplementedError
            if self.log_output:
                assert reduced_value.size == 1
            ...
            if self.save_output:
                uri = (
                    self.out
                    / f"{key}_{self.to_short_interval_string()}_{self.update_counter.cur_checkpoint}.th"
                )
                paddle.save(obj=reduced_value, path=uri)
