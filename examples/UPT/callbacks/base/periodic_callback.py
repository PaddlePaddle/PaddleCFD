...
class PeriodicCallback(CallbackBase):
    def __init__(
        ...
        **kwargs,
        ...
        if (
            type(self)._track_after_accumulation_step
            != PeriodicCallback._track_after_accumulation_step
            or type(self)._track_after_update_step
            != PeriodicCallback._track_after_update_step
        ):
            assert (
                sum(
                    [
                ...
                <= 1
            ), "tracking callbacks can't have multiple interval types"
        assert self.update_counter is not None
        assert (
            type(self).track_after_accumulation_step
            == PeriodicCallback.track_after_accumulation_step
        )
        assert (
            type(self).track_after_update_step
            == PeriodicCallback.track_after_update_step
        )
        assert type(self).after_update == PeriodicCallback.after_update
        assert type(self).after_epoch == PeriodicCallback.after_epoch
        assert (
            type(self).register_sampler_configs
            == PeriodicCallback.register_sampler_configs
        )
        assert (
            type(self)._register_sampler_config_from_key
            == PeriodicCallback._register_sampler_config_from_key
        )

    def __str__(self):
        detail_str = self._to_string() or ""
        return (
            f"{type(self).__name__}({self.get_interval_string_verbose()}{detail_str})"
        )
    ...
    def _register_sampler_config_from_key(self, key, mode):
        ...
        return self.__register_sampler_config(
            dataset=dataset, mode=mode, name=key, collator=collator
        )
    ...
    def __register_sampler_config(self, dataset, mode, name, collator=None):
        ...
        config = InterleavedSamplerConfig(
            sampler=paddle.io.DistributedBatchSampler(
                dataset=dataset, shuffle=False, batch_size=1
            )
            if is_distributed()
            else paddle.io.SequenceSampler(dataset),
    ...
    def track_after_update_step(self, **kwargs):
        if (
            type(self)._track_after_update_step
            == PeriodicCallback._track_after_update_step
        ):
            return
    ...
    def after_update(self, effective_batch_size, **kwargs):
        ...
        if self.should_log_after_sample(
            self.update_counter.cur_checkpoint, effective_batch_size
        ):
    ...
    def updates_till_next_log(self):
        ...
        return (
            ...
            - self.update_counter.cur_checkpoint.update % updates_per_log_interval
        )
    ...
    def iterate_over_dataset(
        self, forward_fn, config_id, batch_size, data_iter=None, use_collate_fn=True
    ):
        ...
        if isinstance(config.sampler, paddle.io.DistributedBatchSampler):
        ...
        self.__sampler_configs_counter = (self.__sampler_configs_counter + 1) % len(
            self._sampler_configs
        )
        data_times = []
        forward_times = []
        ...
        self.logger.info(
            f"{prefix}: data={mean_data_time:.2f} forward={mean_forward_time:.2f}"
        )
    ...
    def _collate_tensors(tensors):
        ...
        return paddle.concat(tensors)

    @staticmethod
    def _collate_result(result, global_dataset_len):
        if isinstance(result[0], dict):
            result = {
                k: PeriodicCallback._collate_tensors([r[k] for r in result])
                for k in result[0].keys()
            }
            result = {
                k: all_gather_nograd_clipped(v, global_dataset_len)
                for k, v in result.items()
            }
        elif isinstance(result[0], list):
            result = [paddle.concat(item) for item in zip(*result)]
            result = [
                all_gather_nograd_clipped(item, global_dataset_len) for item in result
            ]
        elif result[0] is None:
            return None
        else:
            if paddle.is_tensor(result[0]):
                result = paddle.concat(result)
            else:
                result = paddle.tensor(result)
