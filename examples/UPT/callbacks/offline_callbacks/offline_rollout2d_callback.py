...
class OfflineRollout2dCallback(PeriodicCallback):
    ...
    def tensor_to_pil(self, data, progress, lower_bound, upper_bound):
        error_min = data[2]._min()
        error_max = data[2]._max()
        data -= paddle.tensor(
            [lower_bound, lower_bound, error_min], device=data.device
        ).view(-1, 1, 1)
        data /= paddle.tensor(
            [upper_bound, upper_bound, error_max], device=data.device
        ).view(-1, 1, 1)
    ...
    def visualize(self, idx, trajectories, deltas):
        ...
        if sum([self.save_gif, self.save_pngs]) == 0:
            return
        trajectories = paddle.concat([trajectories, deltas], dim=3)
        for i, trajectory in enumerate(trajectories):
            ...
            if self.visualize_velocity_magnitude:
                ...
                velocity_magnitude = paddle.sqrt(paddle.sum(velocity**2, dim=1))
    ...
    def _forward(self, batch, model, trainer):
        ...
        with trainer.autocast_context:
            if self.use_teacher_forcing:
                ...
                predictions = model.rollout_teacher_forced(
                    x=x, geometry2d=geometry2d, velocity=velocity, **self.rollout_kwargs
                )
            else:
        ...
        trajectories = paddle.concat([ground_truth, predictions], dim=3)
        ...
        denormed_movement = (
            denormed_predictions - denormed_predictions.roll(shifts=(-1,), dims=(1,))
        )[:, :-1]
        ...
        results = dict(
            overall_normalized_delta=normalized_deltas.flatten(start_dim=1).mean(
                dim=-1
            ),
            overall_normalized_delta_masked=(normalized_deltas * mask)
            .flatten(start_dim=1)
            .mean(dim=-1),
            overall_denormalized_delta=denormalized_deltas.flatten(start_dim=1).mean(
                dim=-1
            ),
            overall_denormalized_delta_masked=(denormalized_deltas * mask)
            .flatten(start_dim=1)
            .mean(dim=-1),
            overall_denormalized_movement=denormed_movement.flatten(start_dim=1).mean(
                dim=1
            ),
            overall_denormalized_movement_masked=(denormed_movement * mask)
            .flatten(start_dim=1)
            .mean(dim=1),
        )
        if self.save_plots:
    ...
    def _periodic_callback(self, model, trainer, batch_size, data_iter, **_):
        ...
        if self.save_plots:
            paddle.save(
                obj=results["delta_per_channel"].mean(dim=0),
                path=self.out / "plots" / f"PerChannel_{file_identifier}.th",
            )
            paddle.save(
                obj=results["delta_per_timestep"].mean(dim=0),
                path=self.out / "plots" / f"PerTimestep_{file_identifier}.th",
            )
            paddle.save(
                obj=results["delta_per_channel_per_timestep"].mean(dim=0),
                path=self.out / "plots" / f"PerChannelPerTimestep_{file_identifier}.th",
            )
            paddle.save(
                obj=results["delta_per_location"].mean(dim=0),
                path=self.out / "plots" / f"PerLocation_{file_identifier}.th",
            )
