...
class OfflineLagrangianLargeTRolloutMeshLossCallback(PeriodicCallback):
    def __init__(
        ...
        self.time_indicies = paddle.tensor(
            [item for sublist in time_indicies for item in sublist]
        )
    ...
    def _forward(self, batch, model, trainer, trainer_model):
        ...
        all_vel = einops.rearrange(
            all_vel, "bs time n_particles dim -> (bs n_particles) time dim"
        )
        all_vel_target = all_vel[:, (self.time_indicies), :]
        dt = (
            self.data_container.get_dataset().metadata["dt"]
            * self.data_container.get_dataset().metadata["write_every"]
        )
        dx = self.data_container.get_dataset().metadata["dx"]
        dim = self.data_container.get_dataset().metadata["dim"]
        all_vel_target = self.data_container.get_dataset().unnormalize_vel(
            all_vel_target
        )
        all_vel_target = all_vel_target
        vel_predictions = self.data_container.get_dataset().unnormalize_vel(
            vel_predictions
        )
        ...
        relative_norm = (
            (vel_predictions - all_vel_target).norm(dim=3) / all_vel_target.norm(dim=3)
        ).mean(dim=(1, 2))
        results_dict = {
        ...
        if self.rollout_kwargs["save_rollout"]:
            ...
            outpath = (
                self.out
                / f"rollout_results_{str(self.update_counter.cur_checkpoint).lower()}.pt"
            )
            paddle.save(obj=rollout_dict, path=outpath)
        return results_dict
