...
class OfflineCorrelationTimeCallback(PeriodicCallback):
    ...
    def _forward(self, batch, model, trainer, trainer_model):
        ...
        for i in range(batch_size):
            ...
            cur_preds_mean = paddle.mean(cur_preds, dim=1, keepdim=True)
            cur_target_mean = paddle.mean(cur_target, dim=1, keepdim=True)
            cur_preds_std = paddle.std(x=cur_preds, axis=1, unbiased=False)
            cur_target_std = paddle.std(x=cur_target, axis=1, unbiased=False)
            mean_corr_per_timestep = (
                paddle.mean(
                    (cur_preds - cur_preds_mean) * (cur_target - cur_target_mean), dim=1
                )
