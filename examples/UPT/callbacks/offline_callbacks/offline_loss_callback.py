...
class OfflineLossCallback(PeriodicCallback):
    ...
    def _forward(self, batch, trainer_model, trainer):
        use_amp = getattr(trainer, "use_amp", True) 
    
        with paddle.amp.auto_cast(enable=use_amp):
            losses, outputs = trainer_model(
                batch=batch, reduction="mean_per_sample", **self.forward_kwargs
            )
    ...
    def _periodic_callback(
        self, model, trainer_model, trainer, batch_size, data_iter, **_
    ):
        losses, outputs = self.iterate_over_dataset(
            forward_fn=partial(
                self._forward, trainer_model=trainer_model, trainer=trainer
            ),
        ...
        if self.save_losses:
            for loss_name, loss in losses.items():
                ...
                paddle.save(obj=loss, path=self.out / fname)
        for loss_name, loss in losses.items():
            ...
            train_loss = self.writer.log_cache.get(
                f"loss/online/{loss_name}/{self.to_short_interval_string()}", None
            )
        ...
        for name, output in outputs.items():
            assert (
                output.ndim == 1
            ), f"output has to be calculated sample-wise (name={name} shape={output.shape})"
            self.writer.add_scalar(
