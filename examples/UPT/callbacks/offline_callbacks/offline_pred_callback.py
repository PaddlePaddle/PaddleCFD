...
class OfflinePredCallback(PeriodicCallback):
    ...
    def _periodic_callback(
        ...
        x_hat, target = self.iterate_over_dataset(
            forward_fn=partial(
                self._forward, trainer_model=trainer_model, trainer=trainer, model=model
            ),
        ...
        x_hat_uri = (
            self.out
            / f"pred_{self.dataset_key}_{self.update_counter.cur_checkpoint}.th"
        )
        self.logger.info(f"saving predictions to: {x_hat_uri.as_posix()}")
        paddle.save(obj=x_hat, path=x_hat_uri)
        paddle.save(
            obj=target,
            path=self.out
            / f"target_{self.dataset_key}_{self.update_counter.cur_checkpoint}.th",
        )
