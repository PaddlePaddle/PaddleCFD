...
from callbacks.checkpoint_callbacks.checkpoint_callback import \
    CheckpointCallback
from distributed.config import get_rank, is_rank0
...
class KillOnLossSpikeCallback(PeriodicCallback):
    ...
    def load_state_dict(self, state_dict):
        if state_dict is None:
            self.logger.error(
                f"state_dict of KillOnLossSpikeCallback is None on rank {get_rank()} -> skip loading"
            )
            return
    ...
    def _periodic_callback(self, model, **kwargs):
        loss = self.writer.log_cache[
            f"loss/online/total/{self.to_short_interval_string()}"
        ]
        if self.recovery_tolerance_counter > 0:
            if loss < self.best_loss:
                ...
                self.logger.info(
                    f"tolerance: {self.recovery_tolerance_counter}/{self.recovery_tolerance}"
                )
                if self.recovery_tolerance_counter >= self.recovery_tolerance:
                    raise RuntimeError(
                        f"couldnt recover from loss spike within tolerance"
                    )
        elif loss > self.tolerance_factor * self.best_loss:
            if self.recovery_tolerance == 0:
                raise RuntimeError(
                    f"loss is higher than {self.tolerance_factor} * best loss ({loss:.6f} > {self.tolerance_factor * self.best_loss:.6f})"
                )
            self.recovery_tolerance_counter += 1
            self.logger.warning(
                f"detected loss spike -> loss has {self.recovery_tolerance} intervals to recover"
            )
