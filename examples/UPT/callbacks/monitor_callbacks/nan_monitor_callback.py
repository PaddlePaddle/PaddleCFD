...
class NanMonitorCallback(CallbackBase):
    ...
    def _before_training(self, model, **kwargs):
        for name, module in model.named_modules():
            module.register_forward_post_hook(
                hook=self.NanMonitorHook(name, logger=self.logger)
            )
    ...
    class NanMonitorHook:
        ...
        def _log(self, module, module_input, module_output, nan_tensor_name):
            self.logger.error(
                f"encountered nan in module {self.name} ({nan_tensor_name})"
            )
            self.logger.info(f"parameters:")
            for name, param in module.named_parameters():
                self._log_and_print(f"{name}.abs().max(): {param.abs()._max().item()}")
                self._log_and_print(f"{name}.abs().min(): {param.abs()._min().item()}")
            ...
            for i in range(len(module_input)):
                ...
                tensor = tensor[~paddle.isnan(tensor)]
                name = f"module_input[{i}]"
                self._log_and_print(f"{name}.abs().max(): {tensor.abs()._max().item()}")
                self._log_and_print(f"{name}.abs().min(): {tensor.abs()._min().item()}")
        ...
        def __call__(self, module, module_input, module_output):
            ...
            for i in range(len(module_input)):
                if paddle.is_tensor(module_input[i]) and paddle.any(
                    paddle.isnan(module_input[i])
                ):
                    self._log(
                        module=module,
            ...
            if isinstance(module_output, tuple):
                for i in range(len(module_output)):
                    if module_output[i] is None:
                        continue
                    assert paddle.is_tensor(module_output[i])
                    if paddle.any(paddle.isnan(module_output[i])):
                        self._log(
                            module=module,
                            module_input=module_input,
                            module_output=module_output,
