...
class GradientSpikeMonitorCallback(CallbackBase):
    ...
    def _before_training(self, model, **kwargs):
        for name, module in model.named_modules():
            module.register_full_backward_hook(
                self.NanMonitorHook(name, logger=self.logger)
            )
    ...
    class NanMonitorHook:
        ...
        def _log(self, module, module_input, module_output, tensor_name):
            self.logger.error(
                f"encountered high gradient magnitude in module {self.name} ({tensor_name})"
            )
        ...
        def __call__(self, module, grad_input, grad_output):
            ...
            for i in range(len(grad_input)):
                if grad_input[i] is None:
                    continue
                if grad_input[i].dtype == paddle.long:
                    continue
                key = f"grad_input[{i}]"
                ...
                if len(history) < self.num_history_steps:
                    history.append(mag)
                    continue
                history_mean = paddle.stack(list(history)).mean()
