...
class TrainTimeCallback(PeriodicCallback):
    ...
    def _periodic_callback(self, interval_type, **_):
        ...
        for i, (mean_data_time, mean_update_time) in enumerate(
            zip(mean_data_times, mean_update_times)
        ):
            self.writer.add_scalar(
                f"profiling/train_data_time/{i}/{interval_type}", mean_data_time
            )
            self.writer.add_scalar(
                f"profiling/train_update_time/{i}/{interval_type}", mean_update_time
            )
        self.logger.info(
            f"data={list_to_string(mean_data_times)} update={list_to_string(mean_update_times)}"
        )
