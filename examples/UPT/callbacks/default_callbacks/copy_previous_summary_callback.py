    ...
    def _before_training(self, model, **_):
        ...
        for model_name, model in get_named_models(model).items():
            ...
            for initializer in model.initializers:
                ...
                if summary is None:
                    self.logger.info(
                        f"no summary found for initializer of {model_name} (stage_name='{initializer.stage_name}' stage_id={initializer.stage_id}) -> don't copy anything"
                    )
                    continue
                if initializer.stage_name in summaries:
                    self.logger.info(
                        f"duplicate stage_name when copying summaries from {PreviousRunInitializer.__name__} -> using first summary"
                    )
                    if summary != summaries[initializer.stage_name]:
                        self.logger.warning(
                            f"summaries are not the same -> only first summary is copied"
                        )
                    continue
        ...
        for previous_stage_name, summary in summaries.items():
            ...
            for key, value in summary.items():
                ...
                if new_key in self.summary_provider:
                    self.logger.warning(
                        f"'{new_key}' already exists in summary_provider -> skip"
                    )
                    continue
