...
class CopyPreviousEntriesCallback(CallbackBase):
    ...
    def _before_training(self, model, trainer, **_):
        ...
        for model_name, model in get_named_models(model).items():
            ...
            for initializer in model.initializers:
                ...
                entries_uri = self.path_provider.get_primitive_entries_uri(
                    stage_name=initializer.stage_name, stage_id=initializer.stage_id
                )
                if entries_uri is None:
                    self.logger.info(
                        f"no entries found for initializer of {model_name} (stage_name='{initializer.stage_name}' stage_id={initializer.stage_id}) -> don't copy anything"
                    )
                    continue
                if not entries_uri.exists():
                    self.logger.info(
                        f"entries uri {entries_uri.as_posix()} doesn't exist -> don't copy anything"
                    )
                    continue
                ...
                if initializer.stage_name in all_entries:
                    self.logger.info(
                        f"duplicate stage_name when copying entries from {PreviousRunInitializer.__name__} -> using first entries"
                    )
                    if entries != all_entries[initializer.stage_name]:
                        self.logger.warning(
                            f"entries are not the same -> only first entries is copied"
                        )
                    continue
        ...
        if trainer.initializer is not None:
            ...
            if entries_uri is None:
                self.logger.info(
                    f"no entries found for trainer.initializer (stage_name='{trainer.initializer.stage_name}' stage_id={trainer.initializer.stage_id}) -> don't copy anything"
                )
                return
            if not entries_uri.exists():
                self.logger.info(
            ...
            if samples[updates[0]] // updates[0] != trainer.effective_batch_size:
                self.logger.warning(
                    f"found different effective_batch_size when resuming trainer (current={trainer.effective_batch_size} old={samples[updates[0]]}) -> don't copy entries"
                )
                return
            for update in updates:
                ckpt = Checkpoint(
                    epoch=epochs[update], update=update, sample=samples[update]
                )
