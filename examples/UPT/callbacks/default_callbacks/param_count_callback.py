...
class ParamCountCallback(CallbackBase):
    ...
    def _get_param_counts(model, trace=None):
        if isinstance(model, CompositeModelBase):
            ...
            for name, submodel in model.submodels.items():
                subresult = ParamCountCallback._get_param_counts(
                    submodel, trace=join_names(trace, name)
                )
                result += subresult
    ...
    def _before_training(self, model, **_):
        ...
        max_trainable_digits = (
            int(np.log10(total_trainable)) + 1 if total_trainable > 0 else 1
        )
        ...
        for name, tcount, fcount in param_counts:
            name = name or "total"
            self.logger.info(
                f"{format(tcount, tformat)} | {format(fcount, fformat)} | {name}"
            )
        ...
        if self.verbose:
            ...
            for name, param in model.named_parameters():
                self.logger.info(
                    f"{np.prod(param.shape):>10,} {'train' if param.requires_grad else 'frozen'} {name}"
                )
