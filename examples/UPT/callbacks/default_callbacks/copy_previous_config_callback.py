...
class CopyPreviousConfigCallback(CallbackBase):
    ...
    def _should_include_key(key):
        ...
        if (
            ...
            or key.startswith("dist/")
            or key.startswith("code/")
        ):
            return False
        return True
    ...
    def _before_training(self, model, **_):
        ...
        for model_name, model in get_named_models(model).items():
            ...
            for initializer in model.initializers:
                ...
                if config is None:
                    self.logger.info(
                        f"no config found for initializer of {model_name} (stage_name='{initializer.stage_name}' stage_id={initializer.stage_id}) -> don't copy anything"
                    )
                    continue
                if initializer.stage_name in configs:
                    self.logger.info(
                        f"duplicate stage_name when copying configs from {PreviousRunInitializer.__name__} -> using first config"
                    )
                    if config != configs[initializer.stage_name]:
                        self.logger.warning(
                            f"configs are not the same -> only first configs is copied"
                        )
                    continue
        ...
        for previous_stage_name, config in configs.items():
            if previous_stage_name in self.config_provider:
                self.logger.warning(
                    f"'{previous_stage_name}' already exists in config_provider -> skip copying"
                )
                continue
