...
class EmaCallback(PeriodicCallback):
        ...
        for model_path in self.model_paths:
            ...
            for target_factor in self.target_factors:
                for name, param in cur_model.named_parameters():
                    ...
                    self.parameters[key][name].mul_(target_factor).add_(
                        param, alpha=1.0 - target_factor
                    )
            for name, buffer in cur_model.named_buffers():
    ...
    def _save(self, ckpt, model):
        ...
        for model_path in self.model_paths:
            ...
            for target_factor in self.target_factors:
                state_dict = {
                    **self.parameters[model_path, target_factor],
                    **self.buffers[model_path],
                }
                ckpt_dict = dict(
                ...
                paddle.save(
                    obj=ckpt_dict, path=self.path_provider.checkpoint_path / fname
                )
