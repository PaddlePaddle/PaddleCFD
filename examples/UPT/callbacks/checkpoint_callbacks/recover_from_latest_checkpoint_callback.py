...
class RecoverFromLatestCheckpointCallback(PeriodicCallback):
    ...
    def state_dict(self):
        return dict(num_recoveries=self.num_recoveries, best_loss=self.best_loss)
    ...
    def _periodic_callback(self, model, **kwargs):
        ...
        if loss > 2 * self.best_loss:
            if (
                self.max_num_recoveries is not None
                and self.num_recoveries >= self.max_num_recoveries
            ):
                raise RuntimeError(
                    "maximum number of recoveries reached ({self.max_num_recoveries})"
                )
            self.num_recoveries += 1
            self.logger.warning(
                f"loss is higher than 2 * best loss ({loss:.6f} > 2 * {self.best_loss}) -> recover from latest checkpoint (num_recoveries: {self.num_recoveries})"
            )
