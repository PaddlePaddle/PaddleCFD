...
class BestCheckpointCallback(PeriodicCallback):
    def __init__(
        ...
        **kwargs,
        ...
        self.tolerances_is_exceeded = {
            tolerance: (False) for tolerance in tolerances or []
        }
    ...
    def load_state_dict(self, state_dict):
        ...
        if "metric_at_exceeded_tolerance" in state_dict:
            self.metric_at_exceeded_tolerance = state_dict[
                "metric_at_exceeded_tolerance"
            ]

    def _before_training(self, **kwargs):
        if (
            len(self.tolerances_is_exceeded) > 0
            and self.update_counter.cur_checkpoint.sample > 0
        ):
            raise NotImplementedError(
                f"{type(self).__name__} with tolerances resuming not implemented"
            )
    ...
    def _periodic_callback(self, trainer, model, **kwargs):
        assert (
            self.metric_key in self.writer.log_cache
        ), f"couldn't find metric_key {self.metric_key} (valid metric keys={list(self.writer.log_cache.keys())}) -> make sure the callback that produces the metric_key is called at the same (or higher) frequency and is ordered before the {type(self).__name__}"
        metric_value = self.writer.log_cache[self.metric_key]
        if self._is_new_best_model(metric_value):
            self.logger.info(
                f"new best model ({self.metric_key}): {self.best_metric_value} --> {metric_value}"
            )
    ...
    def _after_training(self, **kwargs):
        for tolerance, value in self.metric_at_exceeded_tolerance.items():
            self.logger.info(
                f"best {self.metric_key} with tolerance={tolerance}: {value}"
            )
