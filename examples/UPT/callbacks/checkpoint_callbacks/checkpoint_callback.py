...
from utils.model_utils import get_frozen_param_count, get_trainable_param_count
...
class CheckpointCallback(PeriodicCallback):
    ...
    def _before_training(self, model, **kwargs):
        ...
        self.logger.info(
            f"estimated checkpoint size: {short_number_str(weight_bytes * 3)}B"
        )
        self.logger.info(
            f"estimated weight checkpoint size: {short_number_str(weight_bytes)}B"
        )
        self.logger.info(
            f"estimated optim checkpoint size: {short_number_str(weight_bytes * 2)}B"
        )
        n_checkpoints = 1
        if self.every_n_epochs is not None:
            n_checkpoints += (
                self.update_counter.end_checkpoint.epoch // self.every_n_epochs
            )
        if self.every_n_updates is not None:
            n_checkpoints += int(
                self.update_counter.end_checkpoint.update / self.every_n_updates
            )
        if self.every_n_samples is not None:
            n_checkpoints += int(
                self.update_counter.end_checkpoint.sample / self.every_n_samples
            )
        multiplier = 0
        ...
        if self.save_optim:
            multiplier += 2
        self.logger.info(
            f"estimated size for {n_checkpoints} checkpoints: {short_number_str(n_checkpoints * weight_bytes * multiplier)}B"
        )
