...
class BestMetricSummarizer(StageSummarizerBase):
    ...
    def summarize(self):
        source_updates, source_values = zip(
            *self.all_log_entries[self.source_key].items()
        )
        best_source_value = (
            np.max(source_values)
            if self.source_higher_is_better
            else np.min(source_values)
        )
        best_source_idxs = np.argwhere(source_values == best_source_value).squeeze(1)
        best_source_checkpoints = [
            self._checkpoint_from_update(source_updates[idx])
            for idx in best_source_idxs
        ]
        if len(best_source_checkpoints) > 1:
            self.logger.info(
                f"multiple best_source_checkpoints {best_source_checkpoints}"
            )
        ...
        self.logger.info(
            f"best source metric at checkpoint(s) {best_source_checkpoints}: {best_source_value}"
        )
    ...
    def _log_best_target_metric(self, target_key, source_updates, best_source_idx):
        ...
        assert all(
            self._checkpoint_from_update(source_update)
            == self._checkpoint_from_update(target_update)
        ...
        best_target_value = (
            np.max(target_values) if target_higher_is_better else np.min(target_values)
        )
        best_target_idxs = np.argwhere(target_values == best_target_value).squeeze(1)
        best_target_checkpoints = [
            self._checkpoint_from_update(target_updates[idx])
            for idx in best_target_idxs
        ]
        if len(best_target_checkpoints) > 1:
            self.logger.info(
                f"multiple best_target_checkpoints {best_target_checkpoints}"
            )
        ...
        self.summary_provider[
            f"{target_key}/atbest/{self.source_key}/update"
        ] = checkpoint.update
