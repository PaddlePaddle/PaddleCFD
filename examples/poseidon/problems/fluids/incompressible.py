...
from .normalization_constants import CONSTANTS
...
class IncompressibleBase(BaseTimeDataset):
    def __init__(
        ...
        **kwargs
        ...
        self.density = paddle.ones(1, self.resolution, self.resolution)
        self.pressure = paddle.zeros(1, self.resolution, self.resolution)
        ...
        self.pixel_mask = paddle.tensor([False, False])
        if not self.just_velocities:
            self.pixel_mask = paddle.tensor([False, False, False, True])
        if tracer:
            self.pixel_mask = paddle.cat(
                [self.pixel_mask, paddle.tensor([False])], dim=0
            )
    ...
    def _downsample(self, image, target_size):
        ...
        sel = paddle.logical_and(
            freqs >= -target_size / 2, freqs <= target_size / 2 - 1
        )
        image_hat = paddle.fft.fft2(image, norm="forward")
        image_hat = image_hat[:, :, sel, :][:, :, :, sel]
        image = paddle.fft.ifft2(image_hat, norm="forward").real()
    ...
    def __getitem__(self, idx):
        ...
        inputs_v = (
            paddle.from_numpy(self.reader["velocity"][i + self.start, t1, 0:2])
            .astype(paddle.float32)
            .reshape(2, self.resolution, self.resolution)
        )
        label_v = (
            paddle.from_numpy(self.reader["velocity"][i + self.start, t2, 0:2])
            .astype(paddle.float32)
            .reshape(2, self.resolution, self.resolution)
        )
        if self.transpose:
        ...
        if not self.just_velocities:
            inputs = paddle.cat([self.density, inputs_v, self.pressure], dim=0)
            label = paddle.cat([self.density, label_v, self.pressure], dim=0)
        else:
            inputs = inputs_v
            label = label_v
        ...
        if self.tracer:
            input_tracer = (
                paddle.from_numpy(self.reader["velocity"][i + self.start, t1, 2:3])
                .astype(paddle.float32)
                .reshape(1, self.resolution, self.resolution)
            )
            output_tracer = (
                paddle.from_numpy(self.reader["velocity"][i + self.start, t2, 2:3])
                .astype(paddle.float32)
                .reshape(1, self.resolution, self.resolution)
            )
            if self.transpose:
...
class KolmogorovFlow(BaseTimeDataset):
    def __init__(self, *args, tracer=False, just_velocities=False, **kwargs):
        ...
        self.pressure = paddle.zeros(1, self.resolution, self.resolution)
        X, Y = paddle.meshgrid(
            paddle.linspace(0, 1, self.resolution),
            paddle.linspace(0, 1, self.resolution),
            indexing="ij",
        )
        f = lambda x, y: 0.1 * paddle.sin(2.0 * np.pi * (x + y))
        ...
        if tracer:
            self.label_description += ",[tracer]"
        self.pixel_mask = paddle.tensor([False, False, False])
        if not self.just_velocities:
            self.pixel_mask = paddle.tensor([False, False, False, True, False])
        if tracer:
            self.pixel_mask = paddle.cat(
                [self.pixel_mask, paddle.tensor([False])], dim=0
            )
    ...
    def __getitem__(self, idx):
        ...
        time = t / self.constants["time"]
        inputs_v = (
            paddle.from_numpy(self.reader["solution"][i + self.start, t1, 0:2])
            .astype(paddle.float32)
            .reshape(2, self.resolution, self.resolution)
        )
        label_v = (
            paddle.from_numpy(self.reader["solution"][i + self.start, t2, 0:2])
            .astype(paddle.float32)
            .reshape(2, self.resolution, self.resolution)
        )
        if not self.just_velocities:
        ...
        inputs = (inputs - self.constants["mean"]) / self.constants["std"]
        label = (label - self.constants["mean"]) / self.constants["std"]
        inputs = paddle.cat([inputs, self.forcing], dim=0)
        label = paddle.cat([label, self.forcing], dim=0)
        return {
            "pixel_values": inputs,
            "labels": label,
            "time": time,
