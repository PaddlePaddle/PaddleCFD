...
class Helmholtz(BaseDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.N_test = 512
        self.resolution = 128
        self.file_path = os.path.join(self.data_path, "Helmholtz.h5")
        self.file_path = self._move_to_local_scratch(self.file_path)
        self.reader = self.file_path
    ...
    def __getitem__(self, idx):
        inputs = (
            paddle.from_numpy(self.reader["Sample_" + str(idx + self.start)]["a"][:])
            .astype(paddle.float32)
            .reshape(1, self.resolution, self.resolution)
        )
        inputs = inputs - 1
        b = float(np.array(self.reader["Sample_" + str(idx + self.start)]["bc"]))
        bc = b * paddle.ones_like(inputs)
        inputs = paddle.cat((inputs, bc), dim=0)
        labels = (
            paddle.from_numpy(self.reader["Sample_" + str(idx + self.start)]["u"][:])
            .astype(paddle.float32)
            .reshape(1, self.resolution, self.resolution)
        )
