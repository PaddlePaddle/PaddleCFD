...
def read_cli(parser):
    ...
    parser.add_argument(
        "--config", type=str, required=True, help="Path to config file or JSON string"
    )
    parser.add_argument(
        "--json_config", action="store_true", help="Whether the config is a JSON string"
    )
    parser.add_argument(
        "--wandb_run_name",
        type=str,
    ...
    parser.add_argument(
        "--data_path", type=str, required=True, help="Base path to data."
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
...
def get_num_parameters_no_embed(model):
    """Returns the number of trainable parameters in a Poseidon model without embedding and recovery."""
    out = 0
...
def resolve_runtime_device():
    """Returns the preferred Paddle runtime device for single-card execution."""

    current_device = paddle.device.get_device()
    if current_device != "cpu":
        return current_device

    available_device_types = paddle.device.get_all_device_type() or []
    custom_device_types = set(paddle.device.get_all_custom_device_type() or [])

    if "gpu" in available_device_types:
    ...
    for device_type in available_device_types:
        ...
        if device_type == "ipu":
            return "ipu"
        if device_type in custom_device_types or device_type in {"xpu", "npu"}:
