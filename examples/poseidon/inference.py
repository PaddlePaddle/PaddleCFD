...
import psutil
import wandb
from metrics import lp_error, relative_lp_error
from ppcfd.models.poseidon import ScOT
from problems.base import BaseTimeDataset, get_dataset
from trainer import EvalPrediction, Trainer, TrainingArguments
...
def get_trainer(
    model_path, batch_size, dataset, full_data=False, output_all_steps=False, workers=-1
):
    """
...
def get_trajectories(
    ...
    if isinstance(ar_steps, int):
        ...
        for i in range(ar_steps):
            ...
            trajectories.append(paddle.stack(traj_))
    else:
        ...
        for i in ar_steps:
            ...
            for j in range(len(dataset_)):
                traj_.append(dataset_[j]["labels"])
            trajectories.append(paddle.stack(traj_))
    return paddle.stack(trajectories, dim=1)


def remove_underscore_dict(d):
    return {
        (key[1:] if key.startswith("_") else key): value for key, value in d.items()
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--final_time", type=int, default=None, help="Final time step to end at."
    )
    parser.add_argument(
        "--ar_steps",
        type=int,
    ...
    if len(params.ar_steps) == 1:
        ...
        ar_steps = params.ar_steps
        params.ar_steps = [
            (step / (params.final_time - params.initial_time))
            for step in params.ar_steps
        ]
    dataset_kwargs = {}
    ...
    if params.mode == "save_samples":
        ...
        for run in sweep.runs:
            if run.state == "finished" or params.allow_failed and run.state == "failed":
        ...
        if params.mode == "eval":
            ...
            _, _, metrics = rollout(
                trainer, dataset, ar_steps=params.ar_steps, output_all_steps=False
            )
            data = {
            ...
            predictions, _, _ = rollout(
                trainer, dataset, ar_steps=params.ar_steps, output_all_steps=True
            )
            ...
            for step in range(predictions.shape[1]):
                metrics = compute_metrics(
                    EvalPrediction(predictions[:, step], labels[:, step])
                )
