...
def tile(x, y):
    ...
    Y = paddle.concat(x=[y[i].tile(repeat_times=[tuple(x.shape)[0], 1]) for i in range(tuple(y.shape)[0])], axis=0)
...
class BSNN(nn.Layer):
    ...
    def construct_mask(self):
        ...
        for i in range(2, self.num_layers - 2):
            num_blocks = int(pow(2, i - 1))
            blocksize1 = int(self.width[i] / num_blocks)
            blocksize2 = 2 * self.layers[i + 1]
        ...
        def forward_layer(BSNN_interior, BSNN_boundary, fG_quad, gGn_quad):
            ...
            for k in range(self.ngs_boundary):
                ...
                if not isinstance(self.problem, Stokes):
                    ...
                    Ggrad_boundary = paddle.grad(
                        G_boundary, INPUT_boundary, paddle.ones_like(G_boundary), allow_unused=True
                    )[0][:, :3]
                    ...
                    G_boundary = (
                        Gx_boundary * x_bc_normal[:, [0]]
                        + Gy_boundary * x_bc_normal[:, [1]]
                        + Gz_boundary * x_bc_normal[:, [2]]
                    )
