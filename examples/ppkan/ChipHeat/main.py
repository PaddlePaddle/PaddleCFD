...
import ppsci
from chip_kanonet import ChipKANONet
...
def train(cfg: DictConfig):
    ...
    print(
        f"data range: x: {np.min(points['x'])} ~ {np.max(points['x'])}, y: {np.min(points['y'])}~{np.max(points['y'])}, u: {np.min(points['u'])}~{np.max(points['u'])}, bc: {np.min(points['bc'])}~{np.max(points['bc'])}, bc_data: {np.min(points['bc_data'])}~{np.max(points['bc_data'])}"
    )
    ...
    optimizer = ppsci.optimizer.Adam(
        learning_rate=scheduler, grad_clip=clip, weight_decay=cfg.TRAIN.weight_decay
    )(model)
...
def evaluate(cfg: DictConfig):
    ...
    logger.info("-" * 60)
    logger.info(
        f"Total Parameters: {total_params:,} (Trainable: {sum(p.numel() for p in model.parameters() if not p.stop_gradient):,})"
    )
    logger.info("=" * 60)
