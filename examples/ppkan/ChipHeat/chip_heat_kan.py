...
import ppsci
from ppcfd.models.ppkan import ChipKANONet
...
def train(cfg: DictConfig):
    ...
    print(f"data range: x: {np.min(points['x'])} ~ {np.max(points['x'])}, y: {np.min(points['y'])}~{np.max(points['y'])}, u: {np.min(points['u'])}~{np.max(points['u'])}, bc: {np.min(points['bc'])}~{np.max(points['bc'])}, bc_data: {np.min(points['bc_data'])}~{np.max(points['bc_data'])}")
    ...
    scheduler = ppsci.optimizer.lr_scheduler.Cosine(
        learning_rate=cfg.TRAIN.learning_rate, 
        epochs=cfg.TRAIN.epochs,
        iters_per_epoch=cfg.TRAIN.iters_per_epoch,
        )()
    optimizer = ppsci.optimizer.Adam(learning_rate=scheduler, grad_clip=clip, weight_decay=cfg.TRAIN.weight_decay)(model)
...
def evaluate(cfg: DictConfig):
    # set model
    model = ChipKANONet(**cfg.MODEL)
    # log model's parameter information
    ...
    logger.info("=" * 60)
    logger.info("Model Layer-wise Parameters:")
    for layer, num in layer_params.items():
        logger.info(f"{layer.ljust(20)}: {num:,}")
    
    logger.info("-" * 60)
    logger.info(f"Total Parameters: {total_params:,} (Trainable: {sum(p.numel() for p in model.parameters() if not p.stop_gradient):,})")
    logger.info("=" * 60)
...
@hydra.main(version_base=None, config_path="./conf", config_name="chip_heat_kan.yaml")
