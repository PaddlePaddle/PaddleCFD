...
def plot_predictions(model, X, y, labels_mean, labels_std, output_dir):
    ...
    error_clipped = np.clip(relative_error, 0, 1)
    error_vmax = min(1.0, np.percentile(relative_errorerror_clipped, 95))
    ...
    # relative error
    plt.subplot(3, 1, 3)
    plt.scatter(coords[:,0], coords[:,1], c=error[:, 0], vmin = 0.0, vmax = error_vmax, cmap='hot', s=5)
    ...
    plt.tight_layout()
    plt.title(f'Pressure Prediction, Mean Relative Error: {np.mean(relative_error):.4f}, Max Relative Error: {np.max(relative_error):.4f}')
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
