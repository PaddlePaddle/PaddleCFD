...
for epoch in range(epochs):
    ...
    test_loss = test_model(model=model_rbf, criterion=criterion, c_test=u0_test, y_test=u_test, x=x, batch_size=batch_size)
...
model_bspline = KANONet(width_branch=width_branch, width_trunk=width_trunk, 
                 trunk_min_grid=trunk_min_grid, trunk_max_grid=trunk_max_grid,
                 branch_min_grid=branch_min_grid, branch_max_grid=branch_max_grid,
                 grid_count=grid_count, grid_opt=grid_opt,
                 apply_base_update=apply_base_update, noise_scale=noise_scale, dtype=dtype,
...
for epoch in range(epochs):
    ...
    avg_loss = total_loss / (len(u0_train) // batch_size + 1)
    train_losses.append(avg_loss)
    test_loss = test_model(model=model_bspline, criterion=criterion, c_test=u0_test, y_test=u_test, x=x, batch_size=batch_size)
    test_losses.append(test_loss)
    results.append([epoch+1, avg_loss, test_loss])
    if epoch % 500 == 0:
        print(f'After Epoch {epoch}, Average Train Loss: {avg_loss}, Test Loss: {test_loss}')
