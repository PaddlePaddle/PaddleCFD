...
def train(cfg: DictConfig):    
    logging.basicConfig(filename=os.path.join(cfg.output_dir,
        f'/{cfg.mode}.log'), level=logging.INFO, format=
        '%(asctime)s:%(levelname)s:%(message)s')
    
    model = instantiate_network(cfg)
    optimizer = paddle.optimizer.AdamW(parameters=model.parameters(),
                                       learning_rate=cfg.lr, weight_decay=1e-06)
    ...
    train_dataloader = datamodule.train_dataloader(enable_ddp=cfg.enable_ddp, 
                                                   batch_size=cfg.batch_size)
    
    tmp_lr = paddle.optimizer.lr.CosineAnnealingDecay(T_max=cfg.num_epochs,
    ...
    for ep in range(cfg.num_epochs):
        ...
        for data_dict in train_dataloader:
            try:
                ...
                if idx_batch == 0 and paddle.distributed.get_rank() == 0:
                    msg = (
                        f"Data Loading Time: {data_dict['Data_loading_time'][0]:.2f} seconds. || "
                        )
                    memory_allocated = paddle.device.cuda.memory_allocated(
                        device=device) / (1024 * 1024 * 1024)
                    msg += (
                        f'Memory Usage: {memory_allocated:.2f} GB (forward), ')
            ...
            for i in range(len(cfg.out_keys)):
                key = cfg.out_keys[i]
                st, end = sum(cfg.out_channels[:i]), sum(cfg.out_channels[:i]
                    ) + cfg.out_channels[i]
        ...
        if num_OOM != 0:
            logging.info(
                f'WARNING: {num_OOM} samples OOM, skipping these samples.')
        ...
        if ep == 0 or (ep + 1
            ) % cfg.save_per_epoch == 0 or ep == cfg.num_epochs - 1:
            state = {
                    'model': model.state_dict(), 
                    'lr': optimizer.get_lr(),  
                    'epoch': ep
                     }
            paddle.save(obj=state, path=
                f'{cfg.output_dir}/{cfg.model}_{ep}.pdparams')
...
def evaluate(cfg: DictConfig):
    ...
    datamodule = instantiate_datamodule(cfg, cfg.n_train_num, 0, cfg.n_test_num)
    test_dataloader = datamodule.test_dataloader(enable_ddp=cfg.enable_ddp, 
                                                   batch_size=cfg.batch_size)
    ...
    for i, data_dict in enumerate(test_dataloader):
        ...
        try:
            out_dict, pred, truth = model.eval_dict(device, data_dict,
                loss_fn=loss_fn, decode_fn=datamodule.decode)
            if cfg.save_eval_results:
                save_eval_results(cfg, pred, truth, indices[i], 
                                  decode_fn=datamodule.decode)
            paddle.device.cuda.empty_cache()
        except MemoryError as e:
            if 'Out of memory' in str(e):
                logging.info(
                    f'WARNING: OOM on sample {i}, skipping this sample.')
                if hasattr(paddle.device.cuda, 'empty_cache'):
                    paddle.device.cuda.empty_cache()
                continue
            else:
    ...
    for k, v in eval_dict.items():
        msg += f'{v.item():.4f}({k}), '
    logging.info(msg)
    max_memory_allocated = paddle.device.cuda.max_memory_allocated(device=
        device) / (1024 * 1024 * 1024)
    print(f'Memory Usage: {max_memory_allocated:.2f} GB (MAX).')


def save_eval_results(cfg: DictConfig, pred, truth, centroid_idx, decode_fn
    =None):
    ...
    evals_results = {'pred_pressure': pred_pressure, 'pred_wallshearstress':
        pred_wallshearstress, 'truth_pressure': truth_pressure,
        'truth_wallshearstress': truth_wallshearstress, 'delta_pressure':
        delta_pressure, 'delta_wallshearstress': delta_wallshearstress}
    centroid = np.load(
        f'{cfg.data_path}/{cfg.mode}/centroid_{centroid_idx}.npy')
    ...
    for k, v in evals_results.items():
        np.save(os.path.join(f'{cfg.data_path}/evals_results',
            f'{k}_{centroid_idx}.npy'), v.T)
